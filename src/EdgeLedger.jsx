...
const G = "var(--green)", R_ = "var(--red)", MID = "var(--text)", BLUE = "var(--blue)";
...
const STATUS = {
  ...
  "early-neutral":  { label: "EARLY · NEUTRAL", color: MID, tip: "Too few closed campaigns to judge. Track adherence, not outcome." },
...
function Info({ C, children }) {
  ...
  return (
    <span style={{ display: "inline" }}>
      <button onClick={() => setOpen(v => !v)} style={{ background: "transparent", border: "none", padding: "0 3px", color: C.muted, cursor: "pointer", fontSize: "0.6875rem", borderBottom: "1px dotted var(--rule2)" }}>{open ? "✕" : "ⓘ"}</button>
      {open && <div style={{ fontSize: "0.6875rem", color: "var(--text,#eee)", lineHeight: 1.55, background: "rgba(59,158,255,0.07)", border: "1px solid rgba(59,158,255,0.3)", borderRadius: 9, padding: "8px 11px", margin: "6px 0" }}>{children}</div>}
    </span>
  );
}
...
function Stat({ C, label, value, color, sub, info }) {
  return (
    <div style={{ flex: "1 1 130px", minWidth: 130, background: "var(--w03)", border: "1px solid var(--w08)", borderRadius: 10, padding: "9px 12px" }}>
      <div style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>{label}{info}</div>
      <div style={{ fontSize: "1rem", fontWeight: 500, letterSpacing: "-0.025em", color: color || "var(--text,#fff)", fontVariantNumeric: "tabular-nums", marginTop: 2 }}>{value}</div>
      {sub && <div style={{ fontSize: "0.6875rem", color: C.muted, marginTop: 2, lineHeight: 1.4 }}>{sub}</div>}
    </div>
  );
}
...
function BellMap({ C, pts }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }} fontFamily="ui-monospace,monospace">
      <polygon points={curve.concat([`${W - pr},${pt}`, `${X(12)},${pt}`]).join(" ")} fill="rgba(0,200,5,0.07)" />
      <polygon points={curve.concat([`${W - pr},${H - pb}`, `${X(12)},${H - pb}`]).join(" ")} fill="rgba(255,80,0,0.06)" />
      <polyline points={curve.join(" ")} fill="none" stroke={MID} strokeWidth="1.8" strokeDasharray="7 4" />
      {[0, 1, 2, 3, 4].map(w => <g key={w}><line x1={pl} y1={Y(w)} x2={W - pr} y2={Y(w)} stroke="var(--gridline)" /><text x={pl - 6} y={Y(w) + 3} fill="var(--axis)" fontSize="10" textAnchor="end">{w}×</text></g>)}
      {[20, 30, 40, 50, 60, 70].map(p => <text key={p} x={X(p)} y={H - pb + 14} fill="var(--axis)" fontSize="10" textAnchor="middle">{p}%</text>)}
      <text x={pl - 34} y={pt + 10} fill="var(--axis)" fontSize="9" transform={`rotate(-90 ${pl - 34} ${pt + 60})`}>payoff (avg win ÷ avg loss)</text>
      <text x={(pl + W - pr) / 2} y={H - 4} fill="var(--axis)" fontSize="9.5" textAnchor="middle">win rate → · pale dashed line = breakeven · green side = profitable, red side = losing</text>
      <text x={X(56)} y={Y(2.6)} fill="rgba(0,200,5,0.7)" fontSize="11" fontWeight="700">PROFITABLE SIDE</text>
      <text x={X(15)} y={Y(0.35)} fill="rgba(255,80,0,0.7)" fontSize="11" fontWeight="700">LOSING SIDE</text>
      {pts.filter(p => p.wr != null && p.payoff != null).map((p, i) => (
        <g key={i}>
          <circle cx={X(p.wr)} cy={Y(p.payoff)} r={p.big ? 8 : 5.5} fill="var(--bg2)" stroke={p.color} strokeWidth="2.4" />
      ...
      {entries.map(([lab, v]) => {
        ...
        return (
          <div key={lab} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
            <span style={{ fontSize: "0.6875rem", color: C.muted, fontVariantNumeric: "tabular-nums" }}>{v || ""}</span>
            <div style={{ width: "100%", height: Math.max(2, (v / max) * 74), borderRadius: 3, background: scr ? "rgba(148,163,184,0.5)" : red ? "rgba(255,80,0,0.65)" : "rgba(0,200,5,0.65)" }} />
            <span style={{ fontSize: "0.6875rem", color: C.muted, whiteSpace: "nowrap", transform: "rotate(-28deg)", transformOrigin: "top center", marginTop: 2 }}>{lab}</span>
          </div>
        );
      })}
    </div>
...
export default function EdgeLedger({ C, font, session, setPage }) {
  ...
  const pts = useMemo(() => {
    ...
    return [
      ...
      { label: "SYSTEM", wr: data.verdict?.wr, payoff: data.verdict?.payoff, color: MID, big: true },
    ];
  }, [data]);
  ...
  const monthCard = (label, m) => m && (
    <div key={label} style={{ flex: "1 1 150px", minWidth: 150, background: "var(--w03)", border: `1px solid ${m.net > 0 ? "rgba(0,200,5,0.25)" : "rgba(255,80,0,0.2)"}`, borderRadius: 10, padding: "9px 12px" }}>
      <div style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>{label}</div>
      <div style={{ fontSize: "1rem", fontWeight: 500, letterSpacing: "-0.025em", color: m.net > 0 ? G : R_, fontVariantNumeric: "tabular-nums" }}>{fmt$(m.net)}</div>
      <div style={{ fontSize: "0.6875rem", color: C.muted, marginTop: 3, lineHeight: 1.5 }}>n {m.n} · WR {num(m.wr, 0)}% · PF {num(m.pf)}<br />avg win {fmt$(m.avgW)} / loss {fmt$(m.avgL)}</div>
    </div>
  );
  ...
  return (
    <div className="card" style={{ padding: "12px 16px", marginBottom: 12, border: `1px solid ${st.color}33` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)" }}>Edge Ledger · admin</span>
      ...
      {showInfo && (
        <div style={{ fontSize: "0.6875rem", color: "var(--text,#eee)", lineHeight: 1.6, background: "var(--w04)", border: "1px solid var(--w14)", borderRadius: 10, padding: "10px 12px", margin: "10px 0 4px" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        ...
        <Stat C={C} label="Profit factor" value={num(v.pf)} color={v.pf >= 1.3 ? G : v.pf >= 1 ? MID : R_}
          ...
          sub="target ≥ 1.3" />
        <Stat C={C} label="Edge vs breakeven" value={num(v.edgeRatio) + "×"} color={v.edgeRatio >= 1.2 ? G : v.edgeRatio >= 1 ? MID : R_}
        ...
        <Stat C={C} label="SQN" value={num(v.sqn)} color={v.sqn >= 2 ? G : v.sqn >= 1.6 ? MID : BLUE}
        ...
        <Stat C={C} label="Open book" value={`${ob.riskFree}/${ob.positions} risk-free`} color={ob.riskFree >= ob.positions * 0.6 ? G : MID}
      ...
      {open && (
        <>
          {/* bell-curve map */}
          <div style={{ marginTop: 14, borderTop: "1px solid var(--w06)", paddingTop: 10 }}>
            <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)" }}>The profitability map — where you sit vs the breakeven curve
              <Info C={C}>Every trading system lives on this map: win rate (x) against payoff ratio (y). The pale dashed curve is mathematical breakeven — W = (1−p)/p. Anything above it makes money; below it loses, no matter how good the stock picks are. Watch the SYSTEM dot migrate as the sample grows: up (bigger winners via runners) or right (higher win rate via better entries) both work — May and June show where you came from.</Info>
            </div>
            <BellMap C={C} pts={pts} />
          </div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            ...
            <div style={{ flex: "2 1 240px", minWidth: 220, background: "rgba(59,158,255,0.05)", border: "1px solid rgba(59,158,255,0.25)", borderRadius: 10, padding: "9px 12px", fontSize: "0.6875rem", color: "var(--text,#eee)", lineHeight: 1.55 }}>
              <b>Read:</b> months are REALIZED cash by exit month (campaign slices). The system cohort (white dot) counts only campaigns entered since {data.systemEntry} — its runners are still open and NOT in these numbers yet. Equities-focused estimate; TradeZella owns truth.
            </div>
          </div>

          {/* derisk scorecard */}
          <div style={{ marginTop: 14, borderTop: "1px solid var(--w06)", paddingTop: 10 }}>
            <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)", marginBottom: 8 }}>Derisk-trim scorecard — is the NEW rule earning its keep?</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <Stat C={C} label="Window validation" value={`day ${dk.medDayMFE ?? "—"}`} color={dk.medDayMFE >= 3 && dk.medDayMFE <= 5 ? G : MID}
              ...
              <Stat C={C} label="Trim adherence" value={num(dk.adherencePct, 0) + "%"} color={dk.adherencePct >= 80 ? G : MID}
              ...
              <Stat C={C} label="MFE capture" value={num(dk.avgCapture)} color={dk.avgCapture >= 0.6 ? G : MID}
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 14, borderTop: "1px solid var(--w06)", paddingTop: 10 }}>
            <div style={{ flex: "1 1 300px", minWidth: 280 }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)" }}>System R-distribution
            ...
            <div style={{ flex: "1 1 300px", minWidth: 280 }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)" }}>Monte Carlo — next 100 trades, resampled from YOUR R
              ...
              {mc.retP50 != null ? (
                <div style={{ fontSize: "0.6875rem", color: "var(--text,#eee)", lineHeight: 1.7, marginTop: 6 }}>
                  ...
                  Median max drawdown: <b style={{ color: MID }}>{num(mc.ddP50, 1)}%</b> · worst-case (95th pct): <b style={{ color: R_ }}>{num(mc.ddP95, 1)}%</b><br />
              ...
              ) : <div style={{ fontSize: "0.6875rem", color: C.muted, marginTop: 6 }}>Needs ≥8 closed campaigns with R.</div>}
            </div>
          </div>

          {/* campaign table */}
          <div style={{ marginTop: 12, borderTop: "1px solid var(--w06)", paddingTop: 8 }}>
            <button onClick={() => setTblOpen(x => !x)} style={{ background: "transparent", border: "none", color: C.muted, cursor: "pointer", fontFamily: font, fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, padding: 0 }}>
              {tblOpen ? "▾" : "▸"} System campaigns · audit every number ({(data.campaigns || []).length})
            </button>
            {tblOpen && (
              <div style={{ overflowX: "auto", marginTop: 8 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.6875rem", fontVariantNumeric: "tabular-nums" }}>
                  <thead><tr style={{ color: C.muted, textAlign: "left" }}>
                    {["Ticker", "P&L", "R", "MFE R", "Day of max", "Capture", "Shadow R", "Trim day", "Exit reasons"].map(h => <th key={h} style={{ padding: "4px 8px", borderBottom: "1px solid var(--w14)", fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--faint)", whiteSpace: "nowrap" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {(data.campaigns || []).map((c, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid var(--w06)" }}>
                        <td style={{ padding: "4px 8px", fontWeight: 600 }}>{c.ticker}{c.rescued ? " ·R" : ""}</td>
                        <td style={{ padding: "4px 8px", color: c.pl > 0 ? G : R_, fontWeight: 500, letterSpacing: "-0.025em" }}>{fmt$(c.pl)}</td>
                        ...
                        <td style={{ padding: "4px 8px", color: c.deriskCostR != null ? (c.deriskCostR <= 0 ? G : MID) : undefined }}>{num(c.shadowR)}{c.deriskCostR != null ? ` (${c.deriskCostR > 0 ? "+" : ""}${num(c.deriskCostR)})` : ""}</td>
                ...
                <div style={{ fontSize: "0.6875rem", color: C.muted, marginTop: 6, lineHeight: 1.5 }}>
                  R = banked P&L vs initial risk (entry − first stop) · MFE R = best the trade ever offered · Shadow R = what NEVER trimming would have made (bracket = shadow − actual: green = trim saved, pale = trim cost) · "·R" = rescue. Bars: EOD.
                </div>
              </div>
            )}
          </div>
