...
const gaugeColor = (n, C) =>
  n >= 100 ? C.red : n >= 60 ? C.orange : n < 10 ? "var(--faint)" : C.green;
const gaugeLabel = (n) =>
...
export default function BurstLog({ C, font }) {
  ...
  const s0 = sessions[0];
  const small = { fontSize: "0.75rem", color: C.muted };
  const th = { padding: "8px 10px", fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--faint)", textAlign: "right", whiteSpace: "nowrap" };
  const td = { padding: "9px 10px", fontSize: "0.75rem", fontVariantNumeric: "tabular-nums", textAlign: "right", whiteSpace: "nowrap" };
  const tdn = { ...td, fontWeight: 500, letterSpacing: "-0.025em", fontFamily: "'Geist Mono', ui-monospace, monospace" };
  ...
  return (
    <div style={{ fontFamily: font, paddingBottom: 40 }}>
      ...
      <div style={{ ...small, lineHeight: 1.55, maxWidth: 820, marginBottom: 14 }}>
        <b style={{ color: "var(--muted)" }}>Gauge</b> = froth thermometer (share-volume floor, count only).
        <b style={{ color: "var(--muted)" }}> Tradeable</b> = names above the ≥$20M avg dollar-volume floor.
      ...
      {s0.summary && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
          ...
          <Chip C={C}>tradeable {s0.rows.length}</Chip>
          ...
          <button onClick={() => setCheat(c => !c)} style={{ background: "var(--w06)", border: `1px solid ${C.border}`, borderRadius: 999, color: C.muted, fontFamily: font, fontSize: "0.6875rem", fontWeight: 500, padding: "3px 12px", cursor: "pointer" }}>{cheat ? "hide levels ▴" : "levels ▾"}</button>
        </div>
      )}

      {/* levels cheat card */}
      {cheat && (
        <div style={{ background: "var(--w03)", border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 16px", marginBottom: 14, maxWidth: 780 }}>
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderTop: i ? "1px solid var(--w06)" : "none" }}>
                  <td style={{ padding: "7px 10px 7px 0", fontSize: "0.75rem", fontWeight: 600, color: "var(--text)", whiteSpace: "nowrap" }}>{k}</td>
                  <td style={{ padding: "7px 0", fontSize: "0.75rem", color: C.muted, lineHeight: 1.5 }}>{v}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* daily rows */}
      <div style={{ border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", background: "var(--w02)" }}>
                ...
                return (
                  <React.Fragment key={s.date}>
                    <tr onClick={() => setOpen(isOpen ? null : s.date)} style={{ cursor: "pointer", borderTop: "1px solid var(--w04)", background: isOpen ? "var(--w06)" : "transparent" }}>
                      ...
                      <td style={{ ...tdn, color: "var(--muted)" }}>{fmtCap(sm.medCapB)}</td>
                      <td style={{ ...tdn, color: pctIn == null ? C.muted : pctIn >= 50 ? C.green : C.red }}>{pctIn == null ? "—" : `${sm.inTheme}/${sm.themed} (${pctIn}%)`}</td>
                      <td style={{ ...td, textAlign: "left" }}>
                        {top.length ? top.map(([name, n]) => <span key={name} style={{ marginRight: 8, fontSize: "0.6875rem", color: "var(--muted)" }}>{name} <b style={{ fontWeight: 600, color: C.white }}>{n}</b></span>) : <span style={{ color: C.muted, fontSize: "0.6875rem" }}>—</span>}
                      </td>
                    </tr>
                    {isOpen && (
                      <tr>
                        <td colSpan={7} style={{ padding: "4px 14px 16px", background: "var(--w08)" }}>
                          {/* cap-bucket strip */}
                          <div style={{ ...small, margin: "8px 0 2px" }}>Cap profile: <b style={{ color: "var(--muted)" }}>{summaryLine(s) || "—"}</b>{sm.unmapped ? ` · ${sm.unmapped} theme-unmapped` : ""}</div>
                          <table style={{ borderCollapse: "collapse", width: "100%", marginTop: 8 }}>
                            ...
                            <tbody>
                              {s.rows.map((r, i) => (
                                <tr key={r.t} style={{ borderTop: i ? "1px solid var(--w04)" : "none" }}>
                                  <td style={{ ...td, textAlign: "left", color: C.muted, padding: "6px 8px" }}>{i + 1}</td>
                                  <td style={{ ...td, textAlign: "left", fontWeight: 600, color: "var(--text)", padding: "6px 8px" }}>{r.t}{r.flag && <span title="unadjusted data — possible split, verify" style={{ color: C.orange, marginLeft: 5 }}>⚠</span>}</td>
                                  <td style={{ ...tdn, color: C.green, padding: "6px 8px" }}>+{r.pct}%</td>
                                  <td style={{ ...tdn, color: "var(--text)", padding: "6px 8px" }}>{fmtCap(r.capB)}</td>
                                  <td style={{ ...tdn, color: "var(--muted)", padding: "6px 8px" }}>${r.c}</td>
                                  <td style={{ ...tdn, color: C.muted, padding: "6px 8px" }}>${r.dvM}M</td>
                                  <td style={{ ...td, textAlign: "left", padding: "6px 4px" }}>{gateBadge(r)}</td>
                                  <td style={{ ...td, textAlign: "left", color: r.theme ? "var(--muted)" : C.muted, padding: "6px 8px", fontSize: "0.6875rem" }}>{r.theme || (r.sic ? <i title="SIC industry (not a DeepVue theme)">{r.sic}</i> : "—")}</td>
                                  <td style={{ ...tdn, padding: "6px 8px", color: r.f3 == null ? C.muted : r.f3 >= 0 ? C.green : C.red }}>{r.f3 == null ? "—" : (r.f3 >= 0 ? "+" : "") + r.f3 + "%"}</td>
                                  <td style={{ ...tdn, padding: "6px 8px", color: r.f5 == null ? C.muted : r.f5 >= 0 ? C.green : C.red }}>{r.f5 == null ? "—" : (r.f5 >= 0 ? "+" : "") + r.f5 + "%"}</td>
                                </tr>
                              ))}
                          ...
                          {s.rows50.length > 0 && (
                            <div style={{ marginTop: 12 }}>
                              <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)", marginBottom: 6 }}>50% in 40 days (tradeable · {s.rows50.length})</div>
                              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                {s.rows50.map((r) => (
                                  <span key={r.t} title={`+${r.pct}% / 40d · ${fmtCap(r.capB)} · ${r.theme || r.sic || "—"}`} style={{ fontSize: "0.75rem", fontWeight: 500, color: "var(--text)", background: "var(--w06)", border: `1px solid ${C.border}`, borderRadius: 999, padding: "3px 10px" }}>{r.t} <span style={{ color: C.green }}>+{r.pct}%</span> {r.gate === 1 ? "🟢" : r.gate === 0 ? "🔴" : ""}</span>
                                ))}
                              </div>
                            </div>
                          )}
                          {/* gauge audit */}
                          <div style={{ marginTop: 12 }}>
                            <div style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)", marginBottom: 6 }}>Gauge audit — all {s.gaugeUp.length} study-floor ↑20% names</div>
                            <div style={{ fontSize: "0.75rem", color: "var(--muted)", lineHeight: 1.9, wordBreak: "break-word" }}>{s.gaugeUp.join(" · ") || "—"}</div>
                          </div>
                        </td>
                      </tr>
                    )}
