  ...
  },
  {
    ...
    items: [
      ...
      { c: "Fresh, not extended", star: true,
        s: "It's a 1st or 2nd base off the lows — NOT a late-stage 4th base that's been running for 3–6 months. Quantify it: how far is price above the 50-day MA, measured in units of the stock's own ATR%? ((price − 50MA) ÷ price) ÷ ATR%. Under ~4× = still near the launchpad, room to run. Above 4× = the slingshot is already stretched — statistically you're buying someone else's exit. Big winners get bought near the base, trimmed into 8–10×." },
    ],
  },
  {
    title: "Base Quality",
  ...
  },
  {
    ...
    items: [
      { c: "Range-expansion breakout on volume",
        s: "The stock breaks out with clear range expansion AND a volume surge — judged by PACE, not the day's total. Compare volume traded so far today vs what this stock normally does by the same time of day (e.g. 40%+ of a full normal day within the first 30 minutes = real demand). A breakout without that volume pace is the one that fades — price always fades outside of participation. If the pace isn't there, the correct action is no trade, even if the price level triggers." },
      ...
      { c: "Tight stop — under 1 ADR (ideally < ½)", key: true,
        s: "Your stop (low of day / base low) sits less than one ADR below entry — ideally under half: (entry − stop) ÷ ATR below 0.5. This single number is what makes the R:R explosive AND sets your size — always compute shares BEFORE the order: shares = risk budget ÷ (entry − stop). Example: $5,000 risk ÷ $4.09 stop distance = 1,222 shares — not a round number you liked. An entry a full ADR off the low means the day's fuel is already spent and your stop is fat; skip it and wait for the next coil." },
      { c: "Invalidation defined before entry",
        s: "You know your exact 'I'm wrong' price BEFORE you click buy — a logical level where the setup is broken and you're out, no thinking. Pro upgrade: instead of one all-or-nothing stop, consider three stops of a third each, staggered at ⅓, ⅔ and the full distance to your level — a straight-line failure then costs about 0.67R instead of the full 1R, without ever moving your 'I'm wrong' price." },
      { c: "Not the first 30 minutes (unless volume is extreme)",
        s: "The open is where tight stops get hunted — the first half hour is opening-auction noise, not trend. Delaying entries ~30 minutes filters most unnecessary stop-outs; the one exception is a truly extreme volume surge that can't wait. If the setup is real, it will still be valid at 10:00." },
      { c: "Clear calendar & clean session — no events, max 3 new positions",
        s: "No new entries right before major economic data or into an earnings report you can't cushion — a binary event against a fresh position is a coin flip with your equity. And cap yourself at ~3 new positions per session: roll risk into the market as earlier entries prove themselves, so one bad day can never erase a good week." },
    ],
  },
];
