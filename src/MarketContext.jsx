...
const BANDS = [
  { min: 4, label: "No new risk", color: "var(--red)", dim: "rgba(255,80,0,0.14)", border: "rgba(255,80,0,0.38)", tip: "Very stretched — 4+ daily ranges above the 50-day line. Markets usually snap back from around 5×. Not the time to open new positions." },
  { min: 2, label: "Selective", color: "var(--orange)", dim: "rgba(255,170,5,0.12)", border: "rgba(255,170,5,0.35)", tip: "Stretched. New buys need extra-strong setups and smaller size." },
  { min: 0, label: "Fresh", color: "var(--green)", dim: "rgba(0,200,5,0.12)", border: "rgba(0,200,5,0.32)", tip: "A healthy distance above the 50-day line — normal conditions for new buys." },
  { min: -Infinity, label: "Repair zone", color: "var(--blue)", dim: "rgba(59,158,255,0.12)", border: "rgba(59,158,255,0.35)", tip: "At or under the 50-day line. The strongest buy days often come when the index bounces off a rising 50-day line." },
];
...
const REGIME_META = {
  trend: { icon: "📈", label: "Trending", color: "var(--green)", dim: "rgba(0,200,5,0.10)", border: "rgba(0,200,5,0.3)" },
  chop: { icon: "🌊", label: "Choppy", color: "var(--orange)", dim: "rgba(255,170,5,0.10)", border: "rgba(255,170,5,0.3)" },
  down: { icon: "📉", label: "Downtrend", color: "var(--red)", dim: "rgba(255,80,0,0.10)", border: "rgba(255,80,0,0.28)" },
};
const regimeTip = (r) => {
...
export default function MarketContext({ C, font, defaultExpanded = false }) {
  ...
  const chip = (m) => (
    <span key={m.k} className="term" data-tip={`${m.k}-day moving average: ${m.v.toFixed(2)}. Price is ${m.above ? "above" : "below"} it.`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 999, fontSize: "0.6875rem", fontWeight: 500, whiteSpace: "nowrap", cursor: "help", background: m.above ? "rgba(0,200,5,0.10)" : "rgba(255,80,0,0.10)", border: `1px solid ${m.above ? "rgba(0,200,5,0.28)" : "rgba(255,80,0,0.26)"}`, color: m.above ? "var(--green)" : "var(--red)" }}>
      {m.k}d {m.above ? "▲" : "▼"}
    </span>
  );
  return (
          ...
          {rows.map(r => {
            ...
            return (
              <span key={r.sym} style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                <b style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text)" }}>{r.sym}</b>
                <span style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: "-0.025em", fontFamily: "'Geist Mono', ui-monospace, monospace", color: "var(--text)", fontVariantNumeric: "tabular-nums" }}>{r.price.toFixed(2)}</span>
                {g && <span style={{ fontSize: "0.6875rem", fontWeight: 500, color: g.color, whiteSpace: "nowrap" }}>{g.icon} {g.label}</span>}
                <span style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "-0.025em", fontFamily: "'Geist Mono', ui-monospace, monospace", fontVariantNumeric: "tabular-nums", color: b.color, whiteSpace: "nowrap" }}>{(r.ext >= 0 ? "" : "−") + Math.abs(r.ext).toFixed(1)}× · {b.label}</span>
              </span>
            );
          })}
          <span style={{ fontSize: "0.75rem", color: C.muted, marginLeft: "auto" }}>expand for the full read ▸</span>
        </div>
      )}
      {collapsed ? null : !rows ? (
        <div style={{ fontSize: "0.75rem", color: C.muted, padding: "6px 0" }}>Loading index data…</div>
      ) : (
        <>
          {rows.map(r => {
            const b = bandFor(r.ext);
            return (
              <div key={r.sym} style={{ padding: "8px 0", borderTop: "1px solid var(--w06)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <b style={{ fontSize: "0.875rem", fontWeight: 600, color: "var(--text)" }}>{r.sym}</b>
                  <span style={{ fontSize: "0.875rem", fontWeight: 500, letterSpacing: "-0.025em", fontFamily: "'Geist Mono', ui-monospace, monospace", color: "var(--text)", fontVariantNumeric: "tabular-nums" }}>{r.price.toFixed(2)}</span>
                  {r.regime && (() => { const g = REGIME_META[r.regime]; return (
                    <span className="term" data-tip={regimeTip(r)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 999, fontSize: "0.6875rem", fontWeight: 500, cursor: "help", background: g.dim, border: `1px solid ${g.border}`, color: g.color, whiteSpace: "nowrap" }}>
                      {g.icon} {g.label} Market <span style={{ fontWeight: 500, opacity: 0.75, fontSize: "0.75rem" }}>· {r.streak} trading session{r.streak === 1 ? "" : "s"} {r.side ? "above" : "below"} EMA21{r.regime === "chop" ? " (needs 10 to trend)" : ""}</span>
                    </span>
                  ); })()}
                  <span className="term tipright" data-tip={b.tip} style={{ marginLeft: "auto", display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 1, padding: "4px 10px", borderRadius: 12, cursor: "help", background: b.dim, border: `1px solid ${b.border}`, color: b.color }}>
                    <span style={{ fontSize: "0.75rem", fontWeight: 500, letterSpacing: "-0.025em", fontFamily: "'Geist Mono', ui-monospace, monospace", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>{(r.ext >= 0 ? "" : "−") + Math.abs(r.ext).toFixed(1)}× · {b.label}</span>
                    <span style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.02em", opacity: 0.75, whiteSpace: "nowrap" }}>ATR% Mult from 50-MA</span>
                  </span>
                </div>
