...
const DEEP_DIVES = [
  ...
  },
  {
    ...
    updated: "2026-08-01",
    // v3 2026-08-03 (his ask, twice-refined): sketched AMD arrow-mark + hand-lettered
    // "ADVANCED MICRO DEVICES" above it, same noir family as the TSLA cover. Versioned filename
    // so the CDN can't serve stale art.
    cover: "https://ifahfxsqgmzyxcebslwe.supabase.co/storage/v1/object/public/trade-charts/modelbook/dives/amd-chronicle-cover-v3.png",
  },
  {
...
// Member "Make it a Deep Dive" (Valen 2026-08-03, self-serve — his word: "it's for their own
// purposes… they can publish live"). A member wraps their Study-Book project (or whole book, '*')
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1500, background: "rgba(5,5,10,0.72)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "min(560px, 100%)", maxHeight: "88vh", overflowY: "auto", background: "#0d0d14", border: `1px solid ${C.border}`, borderRadius: 18, padding: "26px 28px", fontFamily: font }}>
        ...
        <input style={inp} value={title} onChange={(e) => setTitle(e.target.value)} placeholder='e.g. "My First Fifty Breakouts"' maxLength={90} />
        <div style={lab}>One-line premise <span style={{ opacity: 0.6, textTransform: "none", letterSpacing: 0 }}>(optional)</span></div>
...
export function DiveIndex({ C, font, dives, onOpen, onMyBook, myCount, isAdmin, onTogglePublish, community = [], onOpenCommunity, uid }) {
...
export function openMyBookPdf(rows, { makerGate, coverTitle, diveTitle, episodesOverride, introText, forceProject, hypothesesOverride } = {}) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>My Model Book — ${today}</title>
    ...
    <div class="page cover">
      ...
      <h1>${diveTitle ? esc(diveTitle) : coverTitle ? esc(coverTitle) : "My Model Book"}</h1>
...
function openProjectBookPdf(rows, coverTitle, { diveTitle, episodesOverride, introText, hypothesesOverride } = {}) {
  ...
  // Member dives (2026-08-03) pass their own diveTitle — their projects are never in the registry.
  const displayTitle = diveTitle || DEEP_DIVES.find((d) => d.project === coverTitle)?.title || coverTitle;
...
// [a=https://…]text[/a] renders a link. esc() has already run, so the href carries no quotes or
// angle brackets; the https-only guard keeps the token from ever emitting a javascript: URL.
// Hrefs come from our own constants — this is a typography token, not user input.
const pbRich = (s) => esc(s)
  .replace(/\[b\]/g, "<b>").replace(/\[\/b\]/g, "</b>")
  .replace(/\[i\]/g, "<i>").replace(/\[\/i\]/g, "</i>")
  .replace(/\[a=(https:\/\/[^\]\s]+)\]([\s\S]*?)\[\/a\]/g,
    (_, href, txt) => `<a class="pblink" href="${href}" target="_blank" rel="noopener">${txt}</a>`);
...
const PLAYBOOK_PARTS = [
  { num: "1", name: "Weather", folio: "Weather", toc: "Part 1 — Weather · read the market first", stats: "Read the market first · pieces 01–03", count: "3 pieces",
    blurb: "Before I look at a single chart, I check the conditions outside. Is this a market where breakouts work, or one where they get sold? The weather never picks the stock for you. It decides how big you trade, and how patient you are for the rest of the day." },
  { num: "2", name: "Neighborhood", folio: "Neighborhood", toc: "Part 2 — Neighborhood · selection", stats: "Selection · pieces 04–07", count: "4 pieces",
    blurb: "Good houses cluster in good neighbourhoods. Money moves into whole groups of stocks, not one stock at a time. So before I judge any chart I find out where the money is actually going. Then I build the nightly routine that puts those stocks in front of me again and again." },
  { num: "3", name: "House", folio: "House", toc: "Part 3 — House · the setups", stats: "The setups · pieces 08–12", count: "5 pieces",
    blurb: "Now the house itself. These are the five chart patterns I buy and sell. Each one is defined tightly enough to say no with. A setup you cannot describe is a setup you cannot repeat." },
  { num: "4", name: "When to walk in", folio: "When to walk in", toc: "Part 4 — When to walk in · execution", stats: "Execution · pieces 13–16", count: "4 pieces",
    blurb: "The right house at the wrong moment is still a loss. This is the door. The trigger that shows buyers are in control, the maths that sets your size, the limits that can stop you entering, and the stop that keeps every mistake small." },
  { num: "5", name: "After you move in", folio: "After you move in", toc: "Part 5 — After you move in · management", stats: "Management · pieces 17–19", count: "3 pieces",
    blurb: "Getting in was one decision. Everything after it is looking after what you now own. Take some profit while buyers are still excited. Trail what is left on a moving average. Add only when the stock proves itself again. This is where good entries turn into good years." },
  { num: "6", name: "Keeping the roof on", folio: "Keeping the roof on", toc: "Part 6 — Keeping the roof on · survival", stats: "Survival · pieces 20–21", count: "2 pieces",
    blurb: "You can do everything else right and still lose the house. Losing streaks are a normal part of this strategy, not a judgement on you. These two pieces are how I cut size by the schedule, stay in the game, and let the repetitions add up." },
];
...
const PLAYBOOK_INDICATORS = [
  ...
  {
    name: "ATR% Multiple From MA",
    url: "https://www.tradingview.com/script/oimVgV7e-ATR-multiple-from-50-MA/",
    what: "One number telling you how far price sits above its 50-day line, measured in the stock's own normal daily moves instead of in plain percent.",
    use: "This is my profit-taking gauge, and it is the honest way to answer \u201cis this stock extended?\u201d Plain percent cannot answer it: two stocks both 10% above the 50-day line are in completely different situations if one normally moves 1% a day and the other moves 7%. Dividing the distance by the stock's normal daily range fixes that and puts every stock on one scale. [b]Under 4 is the launchpad[/b] \u2014 price is near its 50-day line, bases form here, and this is where I am allowed to buy. [b]Around 5[/b] no adds, start tightening. [b]7.5 to 8[/b] is rare air and my trims scale up. [b]Around 10 on a single stock[/b], sell aggressively into the move \u2014 but on an index or a leveraged fund the same extreme arrives at [b]6 to 7[/b], because a basket of stocks never stretches as far as one. A high reading is a trim signal, never a short signal.",
  },
  {
...
const PLAYBOOK_PIECES = [
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `Before I look at a single chart, I check the market as a whole.` },
      { p: `The measure is [b]breadth[/b]. Breadth counts how many stocks made a big up-move today against how many made a big down-move. That gives me one of three answers: the market is strong, mixed, or weak.` },
      { p: `Most stocks move with the market. So this is the first question of my day, not the last.` },
      { h: `Why it matters` },
      { p: `One question decides how I play the whole day. Is this a market where breakouts work, or one where they get sold?` },
      { p: `This check never picks my stocks. It sets two things: how big I trade, and how patient I am.` },
      { p: `The same setup, in the same stock, can be a full-size trade one week and a pass the next. Nothing about the chart changed. The market did.` },
      { h: `How I read it` },
      { p: `I take the daily count of big up-moves against big down-moves, and I compare them over two periods: the month and the quarter. Three readings matter.` },
      { ul: [
        `[b]Both green[/b] — buyers outnumber sellers on both periods. Breakouts are more likely to work. This is when I push.`,
        `[b]A sudden burst of big gainers[/b] after a weak stretch. This often marks a turn. It is why I check every day instead of once a week.`,
        `[b]A flood of heavy down-moves[/b]. On the daily board that is a warning. On the quarterly board, a very low up-count is the panic reading, and it has marked major lows.`,
      ] },
      { h: `What I do with each answer` },
      { ul: [
        `[b]Strong[/b] — the full playbook.`,
        `[b]Mixed[/b] — half size, take profit sooner, only the best setups.`,
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `Big money does not buy one stock at a time. It buys a whole [b]group[/b] at once: chip makers, banks, uranium, whatever is being funded that month.` },
      { p: `You will hear these called groups, themes or sectors. They mean the same thing. This book says [i]group[/i] throughout.` },
      { p: `Every stock belongs to a group, and that group is either being bought or it is not. When a stock's group is near the top of my performance lists, I call that stock [b]in-theme[/b]. It is a status I look up, not a feeling I have.` },
      { h: `Why it matters` },
      { p: `About half of any stock's move comes from its group, not from the stock itself.` },
      { p: `So when a group is being bought, even its third-best chart tends to beat the best chart in a group being sold.` },
      { p: `That means the group tells me as much about the outcome as the chart does. And I can look it up before I enter.` },
      { h: `How I read it` },
      ...
      { ul: [
        ...
        `[b]Strong this week, absent on the month[/b] — new money arriving. I watch for first bases.`,
        `[b]Strong month, fading week[/b] — a leader resting, or a leader ending. The charts of its top names tell me which.`,
        `[b]Today's big gainers from the month's worst groups[/b] — usually just a bounce in something beaten down, not the start of leadership.`,
      ] },
      { h: `What I do with it` },
      { p: `In-theme stocks get the full playbook.` },
      { p: `If the group is on neither list, the bar goes up. The setup has to be excellent, the size is smaller, and I give it less room.` },
      { p: `I check the group before I look at the chart.` },
      { h: `Common mistakes` },
      { p: `[b]Staying in love with one stock while its whole group turns down.[/b]` },
      { p: `[b]Ignoring the rule because the story is exciting.[/b] If the story were real, the group would already show it.` },
      { p: `[b]Deciding a stock's group by instinct.[/b] Which group a stock trades with is something you look up. Never something you decide for yourself.` },
    ],
    take: `Strong stocks live in strong groups. Check the group before the chart, and let the rule say no for you.`,
  },
  {
    no: "03", part: 0, title: `Rotation`, sub: `Where the money is moving now — and the bounce that looks like leadership`,
    toc: `where the money is moving now, and the bounce that looks like leadership`,
    blocks: [

      { h: `What this is` },
      { p: `Rotation is money moving from one group to another.` },
      { p: `Between the whole market and the single stock sits the group table. Every group is ranked against its own recent history, with a second, faster reading for this week.` },
      { p: `Strong on both means the group is leading and speeding up. Those are the groups I hunt in.` },
      { h: `Why it matters` },
      { p: `Rotation turns "the market is fine" into "look here".` },
      { p: `It also carries one distinction that is worth real money. A group can reach the top of the table in two very different ways, and only one of them is leadership.` },
      { p: `A group bouncing off its lows still has sellers waiting above, people who bought higher and want out. A group at new highs has none of them.` },
      { h: `The three numbers I read` },
      { ul: [
        `[b]Rank[/b] — how the group is performing against its own recent history, with a faster reading for the current week on top.`,
      ...
      { ul: [
        ...
        `Inside the group, buy the leader — the highest-ranked name. Not the one that "hasn't moved yet". It has not moved for a reason.`,
        `Watch the weakest name in the group anyway. When it breaks down first, the leader's clock has started.`,
      ] },
      { h: `Common mistakes` },
      { p: `[b]Reading the rank and skipping the distance-to-highs beside it.[/b] That is how a bounce gets bought at full leadership size.` },
      { p: `[b]Hunting in ten groups at once.[/b] That is the same as hunting in none.` },
      { p: `[b]Buying the cheapest name in a great group.[/b] The ranking already told you which name the money chose.` },
    ],
    take: `The rotation table is the map between the market and the stock. Hunt where it is green and near highs. Treat a bounce off the lows as a different, lesser trade.`,
  },
  {
    no: "04", part: 1, title: `Relative strength`, sub: `Buy what is already outperforming, not what looks cheap`,
    toc: `buy what is already outperforming, not what looks cheap`,
    blocks: [

      { h: `What this is` },
      { p: `Relative strength compares a stock against the market over a set period, then ranks it against every other stock.` },
      { p: `It does not tell me how far a stock has travelled on its own. It tells me where it sits in the field.` },
      { h: `Why it matters` },
      { p: `Big buyers leave tracks long before the news explains why. This is where those tracks show up.` },
      { p: `[b]A stock that refuses to make new lows while the market is making them is being bought by someone.[/b]` },
      { p: `The ranking is published every day, it is free, and it moves before the story does.` },
      { h: `How I read it` },
      { ul: [
        `[b]The floor for a candidate[/b] — top 20% of the market at minimum, top 10% preferred. No exceptions for a good story.`,
        `[b]Three periods, not one[/b] — one month, three months, six months. One hot week is noise. Three periods agreeing is a footprint.`,
        `[b]The line against the market[/b] — how a stock behaves on the market's worst days beats any number.`,
      ] },
      { h: `Compare every number to the market` },
      ...
      { kv: [
        ...
        [`The market went down`, `Any stock that is up has already beaten the market. This is when the filter helps you most, and when the fewest stocks pass it.`],
      ] },
      { h: `Picking your minimum` },
      { p: `There is no fixed number. You decide it. But it has one hard condition: [b]it must be stronger than the market.[/b]` },
      { p: `The market fell 6% over the past month. So a stock up 10% has already beaten it by 16%. That is the floor — when the market is at minus 6%, I want at least 10% for the past month. Anything less is not outperformance, it is just a smaller loss.` },
      { p: `Above that floor, the number is really about your time. The stricter you set it, the fewer stocks come back. If you do not have much time tonight, set it higher and chart fewer.` },
      { p: `Here is one screen, run twice. The market was down 6% both times. The only thing I changed is the minimum 1-month gain:` },
      { ul: [
        `[b]Minimum 20%[/b] — 26% better than the market. Twenty-five stocks pass.`,
        `[b]Minimum 30%[/b] — 36% better than the market. Eight stocks pass.`,
      ] },
      ...
      { p: `When two stocks both pass, I look at how they behaved on the market's worst days. A stock that made a higher low while the market made a lower one is showing me the next leader. That is worth more to me than a few points of rank.` },
      { h: `Common mistakes` },
      { p: `[b]Judging a stock's gain on its own.[/b] Up 5% sounds good. But if the market went up 8% over the same period, that stock is behind. It is weak, not strong. Always compare it to the market.` },
      { p: `[b]Buying a falling stock because it looks cheap.[/b] It is cheap because people are selling it. Cheap is not the same as ready.` },
    ],
    take: `The market ranks every stock every day. Read the ranking. Do not argue with it.`,
  },
  {
    ...
    blocks: [
      ...
      { h: `The point of scanning` },
      { p: `There are thousands of stocks listed in the US. On any given day, fewer than fifty are worth my attention.` },
      { p: `A scan does not find me a trade. It takes the whole market down to a watchlist I can actually read.` },
      { h: `How I think about it` },
      { p: `I do not scan for stocks I hope will move. I scan for stocks already moving.` },
      { p: `Strength leads to more strength. Weakness leads to more weakness.` },
      { p: `So I look for the stocks doing what every big winner did before it ran: outperforming the market, building a base, and coming out of a quiet stretch on heavy volume.` },
      { h: `The three scans I run` },
      { p: `I split the work into three buckets. Same three, every evening.` },
      { ul: [
        `[b](1) The movers ladder[/b] — top gainers over one, three and six months. This finds what has already moved.`,
        `[b](2) The growth scan[/b] — sales and earnings growing hard, high relative strength, price already near its 52-week high. This finds [i]why[/i] a stock should keep moving.`,
        `[b](3) The pre-market gappers[/b] — run around 9:25 New York time, when the real list has formed. This is the scan built for surprise catalysts.`,
      ] },
      { h: `Why I run them at night` },
      { p: `I scan in the evening, when the market is closed and nothing is moving.` },
      { p: `Run daily, the same leaders walk past my screen again and again. So my memory does the compounding. By the time one of them triggers, it is not a new idea I have to judge under pressure. It is a stock I already know.` },
      { p: `The scan builds the watchlist. Only the chart builds the trade.` },
      { h: `Scan 1 — the movers ladder, filter by filter` },
      { p: `This is one saved screen, sorted three different ways. Six filters do all the work. Everything else stays on default.` },
      { kv: [
        [`Price over $3`, `Cuts the sub-$3 tier. Thin books, wide spreads, and the games that live down there. I hunt where institutions can actually buy.`],
        [`Market cap over $300M`, `The smallest size a fund can buy. Below this there are no fund flows to follow.`],
        [`ADR over 4%`, `ADR is how much a stock moves in a normal day. It has to move enough to pay for its stop within days. A stock that travels 1% a day cannot.`],
        [`Average volume over 500K shares`, `Enough shares change hands that I can get in and out without moving the price.`],
        [`Price × average volume over $5M`, `The dollar version of the same check. I use both, because each catches what the other misses. Share volume alone lets in cheap lottery tickets. Dollar volume alone lets in thinly traded expensive ones.`],
        [`Sort by performance`, `The ladder itself. Same stocks, three views. Sort by 1 month, then 3 months, then 6 months.`],
      ] },
      { h: `Reading the three sorts together` },
      { p: `The overlap between the sorts is the real signal.` },
      { ul: [
      ...
      { ul: [
        `[b]Price $3 to $10[/b] — removes the junk tier. The leaders I want almost never live under ten dollars.`,
        `[b]Market cap $300M to $1B[/b] — institutional names only.`,
        `[b]ADR 4% to 5-6%[/b] — keeps only the stocks that can pay fast.`,
        `[b]Dollar volume $5M to $20M or more[/b] — my own bar for trading at real size.`,
        `[b]Add a minimum gain that matches the sort[/b] — 1 month at 30% or more, 3 months at 50%, 6 months at 100%. Now the list holds stocks that already proved it, not everything merely ranked.`,
        `[b]Optional: price above a rising 10- and 20-day line[/b] — keeps only what is still going up. This one cuts hardest.`,
      ] },
      { h: `Scan 2 — the growth scan` },
      { p: `You will hear this one called a [b]CANSLIM[/b] scan. If that word is new, here is all it means.` },
      { p: `CANSLIM is a stock-picking checklist that came out of a study of the biggest winning stocks in market history. The researchers asked one question: before these stocks ran, what did they all have in common? Seven things kept showing up, and each letter stands for one of them — growing earnings this quarter, growing earnings each year, something new happening at the company, a small number of shares available, being the leader rather than the laggard, funds buying it, and the overall market going up.` },
      { p: `You do not have to memorise the letters. Notice instead that you already know most of them from this book. Leader not laggard is piece 04. Market going up is piece 01. Funds buying is what the group lists show you.` },
      { p: `So my growth scan is just those ideas turned into filters. The movers ladder finds what has moved. This one finds why it should keep moving.` },
      { p: `Eight conditions, all required at once. A stock passes every line or it is not on the list.` },
      { kv: [
        [`Price over $3`, `The same junk-tier cut the ladder makes.`],
        [`Market cap over $300M`, `Funds cannot build a position below this, so their buying never shows up in the price.`],
        [`Sales growth over 25% versus a year ago`, `The engine. Sales, not profit — profit can be manufactured by cost-cutting for a few quarters, sales cannot. 25% is where a quarter is genuinely different from the year before.`],
        [`Average volume over 500K shares (60 days)`, `The liquidity floor, measured over a longer window than the ladder so one news spike cannot sneak a thin stock through.`],
        [`Price above the 50-day line`, `The stock has to already be going up. This one filter is what stops the screen filling with cheap growth stories still in downtrends.`],
        [`Volatility over 3%`, `It has to move enough to pay for its stop, however good the numbers read.`],
      ...
      { ul: [
        `[b]Sales growth 25% to 40% or more[/b] — every step up cuts the survivor count hard, which is the whole job.`,
        `[b]Float 150M down to 50M or less[/b] — the strongest lever on this screen. Scarcity is what makes the moves violent.`,
        `[b]Volatility 3% to 5% or more[/b] — keeps only the stocks that can pay within days.`,
        `[b]Add price above the 200-day line[/b] — the long-term trend agreeing with the 50-day.`,
        `[b]Lift the volume floor well past 500K[/b] — so every survivor can be traded at real size.`,
      ] },
      { h: `The same kind of scan, on a free site` },
      { p: `You do not need a paid charting account for a growth scan. Finviz does it for free, and the whole screen fits into one link. Click it and the list is already filtered.` },
      { p: `This version looks for bigger companies than the scan above, and it wants proof that funds are already buying.` },
      { p: `[a=https://finviz.com/screener?v=211&p=d&f=cap_midover,fa_salesqoq_high,fa_salesyoyttm_high,sh_avgvol_o2000,sh_curvol_o1000,sh_insttrans_pos,ta_highlow20d_a5h,ta_highlow50d_a5h,ta_volatility_wo4&ft=4&ta=0]Open the saved Finviz screen[/a]` },
      { kv: [
        [`Market cap over $2 billion`, `Much bigger than the scan above. This one only looks where the big funds already are.`],
        [`Sales growth this quarter over 25%`, `Sales are growing fast in the most recent quarter.`],
        [`Sales growth over the past year over 25%`, `And fast across the whole year too, so one good quarter alone cannot get a stock onto the list.`],
        [`Average volume over 2 million shares`, `Enough shares trade each day that you can buy and sell without moving the price.`],
        [`Volume today over 1 million shares`, `It is actually trading today, not just on paper.`],
        [`Institutions are net buyers`, `Funds bought more than they sold. This is the closest a free screener gets to showing you who is buying.`],
      ...
      { p: `[a=https://www.tradingview.com/watchlists/328880788/]Open the VIV focus list on TradingView[/a]` },
      { p: `It is split into two groups, not ranked as one long list:` },
      { ul: [
        `[b]Strong stocks (stalking)[/b] — already leading. I am waiting for a trigger to buy them.`,
      ...
      { p: `[b]Letting the focus list grow past a dozen[/b] because nothing wants to be cut.` },
      { p: `[b]Skipping the evening after a losing day.[/b] That is exactly when the routine is doing the most work.` },
    ],
    take: `Three scans run every night beat forty scans you never run. What the routine really produces is not trades — it is knowing the leaders before they move.`,
  },
  {
    no: "06", part: 1, title: `The earnings staircase`, sub: `Growing sales and profits — the fuel underneath the chart`,
    toc: `growing sales and profits, the fuel underneath the chart`,
    blocks: [

      { h: `What this is` },
      { p: `The staircase is a company's recent quarters laid side by side, oldest to newest.` },
      { p: `What I want to see is sales growth [i]speeding up[/i] — each stair higher than the last — with profit following sales rather than replacing it.` },
      { p: `This is the fuel underneath a chart. It is either there or it is not.` },
      { h: `Why it matters` },
      { p: `The chart triggers every trade in this book. The staircase does a different job.` },
      { p: `It tells me whether a stock is worth holding for weeks and adding to, or whether it is a quick trade I should cut quickly.` },
      { p: `Growth decides how long I hold and how much I add. It never decides when I buy.` },
      { h: `What counts as fuel` },
      { ul: [
        `[b]Speeding up, not just big[/b] — 20% then 60% then 110% beats a flat 50% every quarter.`,
        `[b]Sales leading profit[/b] — profit growing without sales growing is cost-cutting, and cost-cutting runs out.`,
        `[b]A one-sentence story[/b] the market can repeat, attached to a group that is being funded.`,
        `[b]Surprise[/b] — the violent moves come where nobody expected anything. Strong numbers from a stock everyone had forgotten about.`,
      ] },
      { h: `How I use it` },
      { p: `The chart still decides when I buy. The growth decides how much I am willing to sit through.` },
      { p: `Great chart, no growth behind it: I take the quick trade and cut it quickly.` },
      { p: `Great chart, growing every quarter, in a group being bought: that is the one I work.` },
      { h: `Common mistakes` },
      { p: `[b]Quoting a staircase from a headline or from memory.[/b] Build the table yourself.` },
      { p: `[b]Reading one enormous quarter as acceleration[/b] when the quarters around it are flat.` },
      { p: `[b]Buying the fundamentals before the chart gives a trigger.[/b] Fuel is not a signal. It is a reason to be patient once a signal arrives.` },
    ],
    take: `The chart decides when you buy. The growth decides how much you expect from it. Check the actual numbers — never trade growth you have only read about.`,
  },
  {
    no: "07", part: 1, title: `The no-buy list`, sub: `The trades we refuse to take`,
    toc: `the trades we refuse to take`,
    blocks: [

      { h: `What this is` },
      { p: `The no-buy list is a fixed set of deal-breakers, written down before I need them.` },
      { p: `It is not a score to weigh up. One "yes" ends the conversation, no matter how good the chart looks or how loud the story is.` },
      { h: `Why it matters` },
      { p: `The trades you refuse are half your edge.` },
      { p: `Every line on this list was paid for by a loss that felt obvious afterwards. The list exists so I cannot buy the same loss twice.` },
      { h: `I do not buy when…` },
      { ul: [
        `The market check is red. Breakouts are failing more often than they work.`,
        `The stock is below its 200-day line. That line separates a stock in an uptrend from one still repairing damage. I stay on the right side of it.`,
        `The group is on neither list, or the rotation table flags it as a bounce off the lows.`,
        `The stock is stretched — four or more daily ranges above its 50-day line, or more than 5% past the proper buy point. Fresh, not stretched.`,
        `The base is late in the run — a third or fourth base. The same shape means far less than it did the first time.`,
        `The base is wide and loose — deep pullbacks, overlapping bars, no tightening.`,
        `The breakout has no volume behind it.`,
        `Earnings land within five sessions. The exception is when the trade [i]is[/i] the earnings reaction, planned as one.`,
        `My total open risk is already at its limit. There is no room for another stop.`,
        `The honest reason is boredom, a losing day, or wanting to feel like I am doing something. The market owes me nothing.`,
      ] },
      { h: `How I run it` },
      { p: `The list sits above my desk. I run it before the order exists, not after the fill.` },
      { p: `Ten questions take seconds. I answer them in order, out loud.` },
      { p: `One yes means no. Nine clean answers do not outvote one bad one.` },
      { h: `Common mistakes` },
      { p: `[b]Overriding one line because the other nine are clean.[/b]` },
      { p: `[b]Only checking the list on trades that already feel shaky.[/b]` },
      { p: `[b]Rewriting a line mid-session so the trade in front of you passes.[/b] The list gets edited in the journal, on a quiet evening, with evidence.` },
    ],
    take: `The mistake is never the rule. It is the click that ignored it. If you cannot say why you are buying in one sentence, you are not buying.`,
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `VCP stands for [b]volatility contraction pattern[/b]. That sounds technical. It is not.` },
      { p: `It just means a stock's swings are getting smaller. It runs up, pulls back hard, runs up, pulls back a bit less, then a bit less again. The pullbacks shrink each time until the stock is barely moving.` },
      { p: `Then it breaks out. That break is the trade.` },
      { p: `I trade every setup in this book. But in my opinion this one is among the easiest to master, so it is a good place to start. You will see it all through my model book.` },
      { h: `Why it works` },
      { p: `Every pullback removes another group of sellers.` },
      { p: `The first one scares out the nervous holders. The second one shakes out the people who bought that first dip. By the last one, almost nobody left wants to sell.` },
      { p: `So the shrinking pullbacks are sellers running out, happening in plain sight. That is why I can read this pattern before it breaks.` },
      { p: `[b]The tightening is the edge. The breakout is only the confirmation.[/b]` },
      { h: `Stage 1 — the move before the base` },
      { ul: [
        `A real run of 30% or more into the base.`,
        `That run should be clean and steady, not a zig-zag.`,
        `A young trend — the first, second or third base of the run. Not a late one.`,
      ] },
      { h: `Stage 2 — the shrinking pullbacks` },
      { ul: [
        `Each pullback clearly shallower than the one before. Roughly halving as you go left to right: 25%, then 12%, then 6%.`,
        `The last pullback is tight — 10% deep or less. More of a drift than a swing.`,
        `Volume drying up through the base, with the quietest volume of all inside that final pullback.`,
        `Higher lows into the buy point, price riding its rising 10-, 20- and 50-day lines.`,
        `Nice extras: inside bars right before the trigger, and the 10-, 20- and 50-day lines coming together at the buy point.`,
      ] },
      { h: `Stage 3 — the trigger` },
      { p: `I draw a flat line across the highs of that final pullback. That is my buy point.` },
      { p: `Then I want a day that expands 4% or more — a bar visibly bigger than the last five to ten — closing in the top 30% of its range, on volume above the day before.` },
      { p: `Two more conditions. No more than two up-days into it, so I am not buying on day three. And a catalyst within a couple of sessions is a bonus, never a requirement.` },
      { h: `How I trade it` },
      { p: `The nightly scans find the candidates. The three stages qualify them. The entry checklist times the buy: the opening-range trigger on the breakout day, stop at that day's low.` },
      { p: `Until the expansion day arrives, this is a watchlist name and nothing more.` },
      { p: `Size is worked out from the stop, exactly as in piece 14. What the age of the base changes is whether I take the trade at all — a fourth base is on my no-buy list.` },
      { h: `Common mistakes` },
      { p: `[b]Calling one quiet week a VCP.[/b] It has to be a sequence. Without pullbacks that get smaller, it is just a range.` },
      { p: `[b]Buying loose bases because the story is tight.[/b]` },
      { p: `[b]Jumping in before the expansion day exists.[/b]` },
      { p: `[b]Treating a late base like a first one.[/b] The older the trend, the shorter its fuse.` },
    ],
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `A high tight flag has two parts and nothing else.` },
      { p: `[b]The pole[/b] — a violent run. Roughly a double, 90 to 100% or more, inside about eight weeks.` },
      { p: `[b]The flag[/b] — a shallow, tight, quiet drift near those highs while the stock does almost nothing.` },
      { p: `The trade is the break out of that flag. Like the VCP, I find this one easier to master than most — which is why I point newer traders at these two first.` },
      { p: `One thing to be clear about: this is a [i]continuation[/i] setup. The trend is already running and the flag is the pause inside it. It is not a bottom and it is not a turn.` },
      { h: `Why it works` },
      { p: `A stock doubles in weeks. Then it refuses to give any of it back.` },
      { p: `Think about what that means. Not one holder is willing to sell into a 100% gain. Nobody is taking the money.` },
      ...
      { ul: [
        ...
        `[b]The trigger[/b] — a break above the [i]pattern high[/i], not an early poke through the flag line, on rising volume.`,
        `[b]It is the first or second flag[/b] of the run, not a late one.`,
      ] },
      { p: `There is no partial credit here. The moment the flag rounds out or cuts deep, the setup is gone.` },
      { h: `How I trade it` },
      { p: `Because it is a continuation setup, the trend that built the pole has to still be intact when the flag breaks.` },
      { p: `The entry is timed like every other setup: opening-range trigger, volume confirming, stop at the low of the day.` },
      { p: `One thing to watch. The run into it was so steep that price often stretches far above its 50-day line within days. So the profit-taking rules in Part 5 come into play early.` },
      { h: `Common mistakes` },
      { p: `[b]Forcing the label onto a stock that ran 40% and paused.[/b] The pole is a measured number. The label means nothing until you have measured it.` },
      { p: `[b]Tolerating a flag that widens, rounds out, or cuts deep[/b] and calling it "still constructive".` },
      { p: `[b]Buying the flag line instead of the pattern high.[/b]` },
      { p: `[b]Treating a late flag like a first one.[/b] The same shape at the end of a run means nothing like the same thing.` },
    ],
    take: `The pole is the claim; the tight flag is the proof nobody wants to sell it. Deep, loose or rounded is not a flag — it is a top forming.`,
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `This is my second way into a leader I already missed.` },
      { p: `The stock pulls back to its rising short-term moving averages. It dips [i]below[/i] a previous low or an obvious support level. Then it closes back above it.` },
      { p: `The entry is not the pullback. It is not the dip below either. The entry is the moment price takes that level [b]back[/b].` },
      { h: `Why it works` },
      { p: `The dip below does something no buyer can do on his own. It triggers all the stop orders sitting under that obvious level, and it scares out the nervous holders.` },
      { p: `So when price recovers, there is far less selling left in the way.` },
      { p: `It also gives me a tight stop. The low that was just made is the exact level the trade is arguing about. That is a few percent of risk on a leader in the middle of its trend.` },
      { h: `What I need to see` },
      { ul: [
        `[b]Moving averages still rising[/b] — the short ones sloping up, price still above the longer ones. A pullback into flat or falling averages is a different, worse trade.`,
        `[b]A young trend[/b] — the first, second or third pullback. By the fourth, the easy part of the trend is behind us.`,
        `[b]Volume drying up on the way down[/b] — a heavy-volume pullback is selling, not resting, and the level will not hold.`,
        `[b]A real dip below, then a real reclaim[/b] — price has to break the prior low and close back above it, ideally on a reversal bar. No dip means no shakeout. No reclaim means no trade.`,
        `[b]Stop at the pullback low, under one daily range[/b]. If the chart demands a wider stop, the chart is telling me no.`,
        `[b]Everything else still intact[/b] — group still being bought, relative strength still holding, trend unbroken.`,
      ] },
      { h: `How I trade it` },
      { p: `I let it dip below. I let the weak hands get stopped out. Then I buy the move back through the level.` },
      { p: `I buy nothing while price is still falling. The reclaim is the entire signal. If it never comes, there was never a trade.` },
      { p: `One warning. Because the stop is so tight, the maths gives me a big share count for my usual risk. That is exactly why I must not force this setup onto a stock that has not earned it.` },
      { h: `Common mistakes` },
      { p: `[b]Buying while price is still falling.[/b] That turns a tight stop into a very wide one.` },
      { p: `[b]Mistaking a weak bounce in a broken stock for a pullback.[/b] A pullback keeps the uptrend intact. A breakdown has already lost it.` },
      { p: `[b]Taking the fourth or fifth pullback of an old trend as though it were the first.[/b]` },
    ],
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `An episodic pivot is a buyable gap up. In plain terms: the market suddenly has to reprice a stock almost nobody owned.` },
      { p: `Genuinely new information lands on a forgotten stock. It opens 10% or more above yesterday's close. Volume floods in so fast the stock trades a normal day's worth inside the first thirty minutes.` },
      { p: `This is the setup my pre-market gapper scan is built for, and it is why that scan runs before every open.` },
      { h: `Why it works` },
      { p: `These are the rarest setups in this book. Only a handful a year.` },
      { p: `The gap itself is not the move. It is the announcement that the repricing has started, and that usually takes weeks to play out.` },
      { p: `The good news is that everything which makes these work can be checked on the day.` },
      { h: `What I need to see` },
      { ul: [
        `[b]Neglect before it[/b] — flat or basing for months with no big run behind it. A gap on a stock everybody already loves is not a surprise, it is extension risk. And it should come out of an orderly base, not out of a falling knife.`,
        `[b]The first surprise[/b] — no recent gap of this kind on the chart. The first one reprices the stock. The third one is already priced in.`,
        `[b]The gap itself[/b] — 10% or more visible, or a 4%-plus expansion on an earnings day.`,
        `[b]Volume that proves it[/b] — a huge bar that dwarfs the recent ones, at least three times normal by the close, with pre-market volume leading the way.`,
        `[b]It holds the gap[/b] — closes in the upper part of the day's range, above the prior day. Closing back inside the gap is a rejection.`,
        `[b]A stop that fits[/b] — a tight level available within about one to one-and-a-half daily ranges, and clear air overhead with no resistance to chew through.`,
      ] },
      { h: `Two ways in` },
      { ul: [
        `[b]Day one[/b] — the five-minute opening-range high, stop at the day's low.`,
        `[b]The delayed entry[/b] — when the gap is too big to size sanely against the day's low, I wait. I let the first constructive day form, the first tight pause or higher low above the gap, and I use that instead.`,
      ] },
      { p: `Missing the first hour costs very little here. The real move takes weeks.` },
      { h: `Common mistakes` },
      { p: `[b]Buying the open before any range exists[/b], then finding the stop is a third of the position.` },
      { p: `[b]Calling a rehashed headline new information.[/b]` },
      { p: `[b]Taking a gap on a stock that already ran[/b], where the news confirms rather than surprises.` },
      { p: `[b]Holding one that closed back inside the gap[/b], hoping tomorrow fixes it. If it hesitates, it lied.` },
    ],
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: `What this is` },
      { p: `This is the one setup I take from the short side. Shorting means betting a stock goes down, so I make money when it falls.` },
      { p: `It comes in two versions.` },
      { ul: [
        `[b]The parabolic[/b] — a stock has gone vertical, far above every rising moving average, and finally cracks.`,
        `[b]The breakdown[/b] — a former leader has already topped, been sold into, rolled over, and then loses a level it had been leaning on.`,
      ] },
      { p: `Both are the same trade in different clothing. I am selling something that has [i]already[/i] broken.` },
      { h: `Why it works — and the warning first` },
      { p: `Let me say this plainly. This is not shorting strength.` },
      { p: `Shorting a leader because it "looks too high" is how accounts end. A stock can stay expensive far longer than your position can survive.` },
      { p: `The whole idea here is that the top has already happened. The heavy selling has shown up. The moving averages have turned down. The support level has broken. I am trading what comes after.` },
      { p: `Everything below exists to prove the top happened before I take the trade.` },
      { h: `Version 1 — the parabolic` },
      { ul: [
        `A vertical run: 50 to 100% on a big stock, 300% or more on a small one, in days to weeks.`,
        `Three to five or more up days in a row into the top.`,
        `Visibly far above all of its rising moving averages. Obvious on sight, not a matter of opinion.`,
        `[b]Trigger[/b] — the first crack. A big red bar, or a break of the opening range in my direction. Intraday, a failure at the volume-weighted average price confirms it.`,
        `[b]Structure[/b] — stop at the day's extreme, one daily range or less, with enough room down to the 10- or 20-day line for the target to pay.`,
      ] },
      { h: `Version 2 — the failed leader` },
      { ul: [
        `Selling at the top: failed highs and big red high-volume days.`,
        `The averages roll over — price below the 10-, 20- and 50-day lines, with those lines turning down.`,
        `Lower highs forming into the support level, and the 50-day lost with no recovery within a few sessions.`,
        `The stock goes down on days the market goes up. That sign usually comes before the break.`,
        `The level wears out — three or more tests of the same support, with weak low-volume bounces in between that cannot get back to the falling 20-day line.`,
        `[b]Trigger[/b] — it breaks support and [i]closes[/i] below it. Not an intraday poke. On a bigger down bar, volume above the prior day, closing in the bottom 30% of the range. A gap down through the level counts.`,
      ] },
      { h: `You do not have to trade this one` },
      { p: `Everything else in this book is a long. This is the only short, and it is optional.` },
      { p: `In my experience shorts win less often than longs. So if you never take a single one, you have not missed the system. Learn the long setups first and come back to this when they are second nature.` },
      { h: `How I size it` },
      { p: `The short side is smaller by default. Half the per-trade risk of a long, and my total short exposure stays under half of what I would carry long in a healthy market.` },
      { p: `Targets are the 10- and 20-day lines. I am not waiting for a collapse.` },
      { p: `These trades work in exactly the markets where my long positions should already be shrinking. That is why the setup exists at all.` },
      { h: `Common mistakes` },
      { p: `[b]Shorting a strong stock because it is up a lot.[/b] Up a lot is not a signal.` },
      { p: `[b]Selling the intraday dip below support before a close below it[/b], then getting run over when it recovers.` },
      { p: `[b]Taking the first bounce off support as the breakdown.[/b]` },
      { p: `[b]Shorting at the same size you would go long.[/b] The risk is not the same — a short can keep going against you without limit.` },
    ],
    take: `I do not short strength — I sell a chart that has already broken. No top, no trade.`,
  },
  {
    no: "13", part: 3, title: `How we enter`, sub: `Trigger, volume, stop — checked in that order, every time`,
    toc: `trigger, volume, stop, checked in that order`,
    blocks: [

      { h: `What this is` },
      { p: `An entry is not a price. It is the moment buyers show they are in control.` },
      { p: `Three things have to agree before that moment counts: a trigger, a confirmation, and a stop.` },
      { p: `The three are the same on every setup in this book.` },
      { h: `Why it matters` },
      { p: `The entry is the one moment where the risk is completely in your hands.` },
      { p: `A trigger on its own proves very little. Price pokes through levels all day long.` },
      { p: `What separates a real breakout from a poke is how many people showed up. That is why the volume check is not optional. Without it, the prettiest trigger is a trap.` },
      { h: `The three checks` },
      { ul: [
        `[b]Trigger — the five-minute opening-range high.[/b] The first five-minute bar of the day sets a range. The break of its high is the earliest proof of real demand. On the biggest gaps I use the one-minute range instead.`,
        `[b]Confirmation — relative volume, measured honestly.[/b] Today's volume against the same time of day on a normal day, not against a full day's average. What I want: roughly 40% of a full session's volume already done inside the first thirty minutes.`,
        `[b]Stop — the low of the day, placed with the order.[/b] The day's low is where the day's idea died. If that distance is wider than about one daily range, the trade does not fit. Pass, or take fewer shares.`,
      ] },
      { h: `How I run it` },
      { p: `I check all three in order, out loud, before I click. One sentence each.` },
      { p: `All three, or none. A trigger without volume is a pass. A trigger with volume but a stop wider than one daily range is either a smaller position or no position.` },
      { p: `Notice that I know the stop before I know the share count. That is deliberate. The next piece turns that distance into a number of shares.` },
      { h: `Common mistakes` },
      { p: `[b]Buying mid-bar with no trigger.[/b]` },
      { p: `[b]Entering at lunch on the morning's news.[/b]` },
      { p: `[b]Deciding the size before you know the stop.[/b] That is backwards, as the next piece shows.` },
    ],
    take: `Trigger, volume, stop — one sentence each, before the click. If any of the three is missing, so is the trade.`,
  },
  {
    no: "14", part: 3, title: `Sizing is arithmetic`, sub: `Decide what you can lose first; the share count follows`,
    toc: `decide what you can lose first, the share count follows`,
    blocks: [

      { h: `What this is` },
      { p: `Position sizing answers one question: how much do I lose if I am wrong?` },
      { p: `I decide that number before the trade exists. Then the share count works itself out.` },
      { p: `Size comes out of the risk. Not out of how much I like the trade.` },
      { h: `The formula` },
      { p: `[b]Shares = (Account × Risk%) ÷ (Entry − Stop)[/b]` },
      { p: `A $100,000 account risking 0.3% is $300 of risk. Entry $50, stop $48. That is $2 per share. $300 ÷ $2 = 150 shares.` },
      { p: `I do it out loud, every time, before the order exists.` },
      { h: `My anchors` },
      { ul: [
        `[b]Per-trade risk: a fraction of one percent.[/b] I anchor at a quarter to a third of a percent. Tighter after losses. Never above one percent for any reason.`,
        `[b]Wilder stocks size themselves down automatically.[/b] A wilder stock has a wider stop, and a wider stop means fewer shares. Same risk, different share counts. That is the system working, not a mistake.`,
        `[b]Position size is an output, never a target.[/b] "How many shares can I afford" is the question that ends accounts.`,
      ] },
      { h: `The other half nobody thinks about` },
      { p: `Your risk percentage decides how much money you can lose. It does not decide how big your position is.` },
      { p: `That comes from the second number in the formula: the distance from your entry to your stop. The two work together.` },
      { p: `Here is why it matters. Same $100,000 account. Same 0.3% risk, so $300 on the line every time. Same $50 stock. The only thing that changes is how far away the stop sits:` },
      { kv: [
        [`Stop $1.50 away (3%)`, `I can buy 200 shares. If the stock goes up 25%, I make $2,500. That is 8 times what I risked. If it goes up 50%, 17 times.`],
        [`Stop $1.00 away (2%)`, `I can buy 300 shares. The same 25% move now makes $3,750 — 12 times what I risked. At 50%, 25 times.`],
        [`Stop $0.75 away (1.5%)`, `I can buy 400 shares. The same 25% move makes $5,000 — 17 times what I risked. At 50%, 33 times.`],
      ] },
      { p: `Read that again. I risked exactly $300 every time. The stock made exactly the same move every time.` },
      { p: `[b]But the closer my stop was, the more I made. Half the distance, double the profit.[/b]` },
      { h: `So how do you get a closer stop?` },
      { p: `Not by picking a smaller number. The stop goes where the trade is proven wrong — under the low of the day. You cannot move it closer just because you want more shares.` },
      { p: `You get a closer stop by [b]buying closer to it[/b].` },
      { p: `Buy the moment the trigger happens, while the low of the day is still right below you. Wait an hour and the stock has run — now the low of the day is twice as far away. Same trade, half the shares, half the payoff.` },
      { p: `That is exactly why the entry check exists: only take the trade if the low of the day is within about one normal day's range. A good setup taken late is a worse trade than the same setup taken on time.` },
      { h: `One warning` },
      { p: `A closer stop is also easier to hit. Stocks wobble.` },
      { p: `If you squeeze the stop just to buy more shares, you will get knocked out of trades that were right.` },
      { p: `The stop has to be both close [i]and[/i] in the right place — under a real level on the chart, not at a number you chose because you liked the share count.` },
      { h: `How I run it` },
      { p: `The order never changes: setup, then stop, then arithmetic, then order.` },
      { p: `If the maths gives me a share count that feels too small to bother with, the real answer is that the stop is too wide for my account. Not that the risk percentage should go up.` },
      { p: `That percentage moves for one reason only: a drawdown.` },
      { h: `Common mistakes` },
      { p: `[b]The same share count on every trade regardless of stop distance.[/b]` },
      { p: `[b]Sizing up to win it back.[/b]` },
      { p: `[b]Confusing a big position with a big idea.[/b]` },
    ],
    take: `Two numbers set your size: what you risk, and how far the stop is. Fix the loss first, buy close to the stop, then work out the shares.`,
  },
  {
    no: "15", part: 3, title: `The three limits`, sub: `One trade, all trades together, and total money in the market`,
    toc: `one trade, all trades together, and total money in the market`,
    blocks: [

      { h: `What this is` },
      { p: `Three different ceilings protect the account. They are not the same number, and people mix them up constantly.` },
      { ul: [
        `[b]Risk[/b] — what one stop-out costs you.`,
        `[b]Total open risk[/b] — what all your open positions cost you if they all stopped out today.`,
        `[b]Exposure[/b] — how much money you have in the market at all.`,
      ] },
      { p: `Three questions, three answers, three separate limits.` },
      { h: `Why you need all three` },
      { p: `Each one protects against a different disaster.` },
      { p: `The first stops one trade from doing real damage. The second stops a single bad day, where everything falls at once, from doing it. The third stops the slow bleed of being too heavily invested in a weak market.` },
      { p: `Any one of them can block a trade the other two would have allowed.` },
      { h: `Limit one — risk on a single trade` },
      { p: `The share of the account one stop-out is allowed to cost. The sizing formula in the last piece takes care of it.` },
      { h: `Limit two — total open risk` },
      { p: `Add up every open position's distance to its stop. That total is what today costs you if [i]everything[/i] stops out at once.` },
      { p: `I cap it at roughly 2% of the account.` },
      { p: `One catch: stocks that move together count as one. Three positions in the same group is really one trade held in three names.` },
      { h: `Limit three — total exposure` },
      { p: `How much of the account is invested overnight at all. My standing ceiling is roughly 30% of the account.` },
      { p: `This counts [i]every[/i] overnight position, including the ones whose stops are already at break-even. A stock can gap open below your stop and skip straight past it. Risk-free is not exposure-free.` },
      { p: `Inside that ceiling, the number moves with the market check. Full when the market is strong. A fraction when it is mixed. Near zero when it is weak.` },
      { h: `How I run it` },
      { p: `I answer all three before I enter, in that order.` },
      { p: `If a new setup would push me over the open-risk cap, it waits. Or I tighten a stop on something I already hold to make room.` },
      { p: `As stops move up, open risk falls by itself and the room comes back without me doing anything.` },
      { h: `Common mistakes` },
      { p: `[b]Treating the three as one number.[/b] This is how a trader who feels disciplined takes an undisciplined loss. A comfortable-looking amount of money invested can still be carrying three times the intended risk.` },
      { p: `[b]Waving through a fourth position in the same group and calling it diversification.[/b]` },
      { p: `[b]Going over a limit because the setup looks excellent.[/b] The limits exist for exactly the trades that feel worth breaking them for.` },
    ],
    take: `Risk on one trade, risk across all of them, and how much you hold overnight. Three questions before every entry — three chances for the rules to save you.`,
  },
  {
    no: "16", part: 3, title: `Cutting losers`, sub: `Small when wrong, instantly`,
    toc: `small when wrong, instantly`,
    blocks: [

      { h: `What this is` },
      { p: `The stop goes where the trade is proven wrong.` },
      { p: `That is the day's low that defined the entry, or the pullback low, or the level that undoes the whole idea if it breaks. Never where you simply stop feeling comfortable.` },
      { p: `It is a price on the chart, decided before you own anything. It is the only part of a trade you fully control.` },
      { h: `Why it matters more than anything else here` },
      { p: `You can be average at everything else in this book and survive if you get this piece right. Get this one wrong and being excellent everywhere else will not save you.` },
      { p: `The maths of losses is lopsided:` },
      { ul: [
        `Down 10% needs 11% to get back.`,
        `Down 25% needs 33%.`,
        `Down 50% needs a double.`,
      ] },
      { p: `Small losses are the cost of doing business. Big ones take months to undo.` },
      { h: `My rules` },
      { ul: [
        `The stop goes in with the entry order. A trade without a stop sitting in the market is not a position — it is a guess you are paying for.`,
        `Width ceiling: about one daily range. Wider means the trade does not fit. Fewer shares, or pass.`,
      ...
      { h: `The signal before the stop` },
      { p: `There is a second signal worth acting on before the stop ever triggers.` },
      { p: `The trades that work tend to work [i]straight away[/i].` },
      { p: `A new position that just sits there going nowhere is telling you something. Cutting it back early is a decision, not a panic.` },
      { h: `Common mistakes` },
      { p: `[b]Widening the stop to give the trade room.[/b] That is the trade telling you it does not fit. The answer is fewer shares, not more rope.` },
      { p: `[b]Cancelling the resting order to "watch it manually".[/b]` },
      { p: `[b]Re-entering minutes after the stop with no new trigger.[/b]` },
      { p: `[b]Treating a stop-out as a judgement on you.[/b] It is just what it costs to find out.` },
    ],
    take: `Decide what wrong looks like before you enter, then honour it without arguing. The skill is not being right — it is being small when you are wrong.`,
  },
  {
    no: "17", part: 4, title: `Trim into strength`, sub: `Taking profit while the stock is still going up`,
    toc: `taking profit while the stock is still going up`,
    blocks: [

      { h: `What this is` },
      { p: `Trimming means selling part of a winner while it is still going up.` },
      { p: `The point is that the rest can keep running without your whole position depending on one pullback.` },
      { p: `Two things tell me when: the first few days after a breakout, and how far price has stretched above its 50-day line.` },
      { h: `Why it matters` },
      { p: `When a breakout works, it usually pays fast. A sharp move over the first three to five days.` },
      { p: `After that, the odds of a pullback or a stall go up.` },
      { p: `The traders who never take anything off are the ones who watch their best entries give it all back.` },
      { h: `Trigger one — the first few days` },
      { p: `When the trade is up 3 to 5 times what I risked, I sell 30 to 50% of it. If that has not happened by day three to five, I do it anyway on the clock.` },
      { p: `Then I move the stop on what is left to break-even.` },
      { p: `Now the trade cannot lose. That changes how you hold it more than any technique will.` },
      { p: `The profit trigger leads. The clock is only the backstop, never the other way round.` },
      { h: `Trigger two — how stretched it is` },
      { p: `The second trigger needs one indicator, and it is the most useful number on my chart for taking profit.` },
      { p: `It measures how far price sits above its 50-day line, but not in plain percent. Plain percent lies to you.` },
      { p: `Think about two stocks, both sitting 10% above their 50-day line. The first one normally moves 1% a day. The second normally moves 7% a day. The first is enormously stretched. The second has barely gone anywhere. Same 10%, completely different situations.` },
      { p: `So instead of the raw percent, I divide it by how much that stock moves on a normal day. That gives me a multiple, and it puts every stock on one scale.` },
      { p: `[b]Distance above the 50-day line ÷ the stock's normal daily range = the multiple.[/b]` },
      { p: `A reading of 5 means price is five normal days' worth of movement above its 50-day line. That means the same thing on a quiet stock as on a wild one, which is exactly why it works.` },
      { h: `The bands I use` },
      { kv: [
        [`Under 4 — the launchpad`, `Price is close to its 50-day line. This is where bases form and where I am allowed to buy. Fresh, not stretched.`],
        [`Around 5 — stretched`, `No adds here, and I start tightening. The easy part is behind it.`],
        [`7.5 to 8 — rare air`, `Genuinely unusual. This is where my trims start scaling up.`],
        [`Around 10 on a stock`, `Sell aggressively into the move.`],
        [`6 to 7 on an index or leveraged fund`, `The same "extreme" level, but it arrives sooner. Indexes and leveraged funds are made of many stocks, so they simply do not stretch as far as a single one. Do not wait for 10 on these — it is not coming.`],
      ] },
      { h: `One thing that surprises people` },
      { p: `A high reading is not a sell signal. It is a trim signal. There is a difference.` },
      { p: `Stretched stocks tend to go quiet for about a week and then outperform for months afterwards. So the stretch is a sign of quality, not a sign of a top.` },
      { p: `That is why I trim into it and never short it. And it is why I never chase it either — buying at 8 means my stop is eight normal days away, which is the opposite of piece 14.` },
      { p: `My own record says the same thing. Across 200 of my trades, exits taken at 7 times or deeper won 76% of the time and averaged +10.7%. Exits taken between 2 and 5 won 51% and averaged +2.3%.` },
      { h: `The judgement, in one question` },
      { p: `Is this a healthy trend or a stretched one?` },
      { p: `A healthy trend grinds upward with normal rests along its moving averages. Hold it.` },
      { p: `A stretched one goes near-vertical, closes right at the highs, and volume keeps building. Take some off.` },
      { p: `That difference is measurable. So measure it rather than guessing.` },
      { h: `Common mistakes` },
      { p: `[b]Selling on a feeling a day or two early[/b], then watching the best part of the move happen without you.` },
      { p: `[b]Taking the trim and forgetting the other half.[/b] Moving the stop to break-even is what makes the trim worth taking.` },
      { p: `[b]Holding the whole position through a vertical run because it "still looks strong".[/b] That is exactly what the multiple is there to catch.` },
    ],
    take: `Sell some while everyone is excited, and hold the rest on the rules. Selling it all gives up the big winner; holding it all gives back the gain. The answer is a partial.`,
  },
  {
    no: "18", part: 4, title: `Trail the rest`, sub: `One moving average, daily closes only`,
    toc: `one moving average, daily closes only`,
    blocks: [

      { h: `What this is` },
      { p: `The runner is the part of the position I keep after taking profit.` },
      { p: `I manage it with one trailing moving average, matched to how fast the stock moves, read on daily closes only.` },
      { p: `That line is not a target. It is simply the level that, once lost, ends the trade.` },
      { h: `Why it matters` },
      { p: `The runner is where a good year separates from an average one.` },
      { p: `You cannot know in advance which winner keeps going for months. So every runner gets its chance to prove it — judged by a line on the chart, not by an opinion.` },
      { h: `How I do it` },
      { ul: [
        `[b]The lines[/b] — the 9- and 21-day exponential moving averages for the trail itself, read against the 10-, 20- and 50-day simple averages that show the trend. Fast movers follow the faster line. Steadier ones follow the slower.`,
        `[b]Pick one line and stick with it.[/b] Choose it when you take profit. Swapping to a slower line later is just an excuse to keep holding.`,
        `[b]Daily closes only.[/b] Quick dips through the line during the day are how the market shakes out impatient holders. Only the close counts.`,
        `[b]The trade ends when the closes say so[/b] — the first clear daily close below the line, or the pattern of higher lows breaking.`,
      ] },
      { h: `How I run it` },
      { p: `A quick dip below the line that closes back above it is not an exit. Allowing that is what keeps me in the trends worth holding.` },
      { p: `I check the line once, after the close, and decide the next day then.` },
      { p: `Nothing about a runner is decided in the middle of the day.` },
      { h: `Common mistakes` },
      { p: `[b]Using the fastest line on a slow stock[/b], and getting shaken out of a good trend by normal noise.` },
      { p: `[b]Tightening the stop because you are bored.[/b]` },
      { p: `[b]Selling a runner because it is "up a lot".[/b] Being up a lot is the whole point.` },
    ],
    take: `One line, daily closes, and forgive the quick dips. Give your winners every chance to keep going.`,
  },
  {
    no: "19", part: 4, title: `Adds and pyramids`, sub: `Adding to a winner, only when it proves itself again`,
    toc: `adding to a winner, only when it proves itself again`,
    blocks: [

      { h: `What this is` },
      { p: `An add is a second or third buy into a position that is already working. A pyramid is a series of adds, each one smaller than the last.` },
      { p: `Either way you end up with one position, at one average price, with one risk number. Never a pile of separate trades in the same stock.` },
      { h: `Why it matters` },
      { p: `Adding is how a good position becomes a great one. It is also how a careless one dies.` },
      { p: `The difference is entirely in [i]when[/i] you do it.` },
      { p: `An add made when the chart gives a fresh reason increases the position without increasing the risk. An add made on a feeling increases both.` },
      { h: `The sizing` },
      { p: `Each add is smaller than the one before: a full first buy, then half to three-quarters of it, then a quarter to half.` },
      { p: `That keeps my average price near the original entry.` },
      { p: `Every add also forces the stop on the whole position higher, so the total risk never grows. If the stop cannot move up enough to keep the maths working, I am not allowed the add. How strongly I feel about it does not enter the calculation.` },
      { h: `Where the adds happen` },
      { ul: [
        `The first tight pause against the short moving averages after the initial move. The break out of that pause is the trigger.`,
        `The next base — a stair-step pause and a fresh expansion. Each new coil is a new contract.`,
      ] },
      { p: `The rule fits in one sentence: add only when the stock proves itself again, at a fresh point on the chart, with the whole position's risk still inside the three limits.` },
      { h: `Common mistakes` },
      { p: `[b]Adding into near-vertical days far above the moving averages.[/b] That is the zone where piece 17 says sell, not buy. You add when a stock pauses. You sell when it stretches. Getting those two backwards is the whole difference between building a position and blowing one up.` },
      { p: `[b]Adding without moving the stop[/b], which quietly turns one disciplined position into three undisciplined ones.` },
    ],
    take: `The stock earns the add; you do not decide it on a feeling. The chart triggers it and the risk maths approves it.`,
  },
  {
    no: "20", part: 5, title: `Cutting size in a losing streak`, sub: `Decided in advance, so it never has to be argued about`,
    toc: `decided in advance, so it never has to be argued about`,
    blocks: [

      { h: `What this is` },
      { p: `This is my written-in-advance response to a run of losses: a schedule that cuts trade size in steps as the drawdown gets deeper.` },
      { p: `Because it is decided beforehand, there is nothing to argue about in the moment.` },
      { p: `Losing streaks are a normal feature of this strategy. They are not a judgement on you.` },
      { h: `Why it matters` },
      { p: `Four, six, even ten losses in a row will happen inside a winning system.` },
      { p: `What separates the traders who last from the ones who do not is how big they were trading when the streak arrived, and how quickly they cut back while it lasted.` },
      { h: `The alarm` },
      { p: `Several stop-outs in a row, or the feeling that you are giving profits back.` },
      { p: `When that happens I stop trading and write up what happened before placing another order.` },
      { p: `That feeling is the alarm. Trades taken just to feel like you are doing something are what keep a streak going.` },
      { h: `The size ladder` },
      { p: `As the month's drawdown deepens, I cut new-trade size in steps:` },
      { ul: [
        `[b]Down 3%[/b] — trade at three-quarter size.`,
        `[b]Down 5%[/b] — half size.`,
        `[b]Down 7%[/b] — quarter size.`,
        `[b]Down 10% from peak equity[/b] — close everything, stand down, and spend a full day in the journal before the next trade.`,
      ] },
      { p: `No debate, no exceptions. The schedule decides so I do not have to.` },
      { h: `The review` },
      { p: `I pull up my last twenty trades and ask three things. Were the setups genuinely good, or stretched? Did I respect the limits, or argue with them? Had the market check already told me no?` },
      { p: `The answers are always obvious in hindsight. The whole job of the protocol is to force that hindsight [i]now[/i].` },
      { h: `How I run it` },
      { p: `Streaks make you want to do more. The answer is always less: fewer names, smaller size, shorter sessions.` },
      { p: `Tomorrow's size is set by last night's numbers, not by how this morning's market feels.` },
      { p: `Size comes back the same way it left, one step at a time, as the equity curve earns it back. You cannot think your way back to even. You size your way back.` },
      { h: `Common mistakes` },
      { p: `[b]Trading bigger to win it back.[/b] The one move that turns a drawdown into a hole.` },
      { p: `[b]Skipping the review day because the setups "look too good to miss".[/b]` },
      { p: `[b]Treating the size ladder as a suggestion.[/b]` },
      { p: `[b]Declaring the system broken at four losses[/b] when its own history says four losses are ordinary.` },
    ],
    take: `Cut size early and by the schedule, so you are still in good shape when conditions come back. Survival first — the recovery follows from it.`,
  },
  {
    no: "21", part: 5, title: `Progress over perfection`, sub: `Repetitions add up; opinions don't`,
    toc: `repetitions add up, opinions don't`,
    blocks: [

      { h: `What this is` },
      { p: `The most common way to fail with a system like this is not breaking the rules. It is freezing while following them.` },
      { p: `The answer is simple. Take the trade the system flagged, at a size you can survive being wrong at, and let the number of correct repetitions do the work that waiting for certainty never will.` },
      { h: `Why it matters` },
      { p: `The edge in this book is small on any single trade. It only shows up over a long run of them.` },
      { p: `The traders who compound are not the ones who were most often right. They are the ones who kept showing up, at survivable size, doing the same boring correct things.` },
      { h: `What actually helps` },
      { ul: [
        `[b]Trade small when unsure.[/b] Shrinking the stakes of one decision cures most paralysis.`,
        `[b]Write the trigger and the stop the night before.[/b] When the alert fires the decision is already made. You are just executing yesterday's clarity.`,
        `[b]Log the trades you did not take[/b] beside the ones you did. That list teaches you faster than your profit and loss does. Most rejected setups were fine, and some rejected rules were right.`,
        `[b]Judge the week on process, not on the result.[/b] Did you take the setups the system flagged? Did you cut what it said to cut? Did you respect the three limits? Three yeses is a good week, whatever the number says.`,
      ] },
      { h: `How I run it` },
      { p: `Being unsure is a reason to trade smaller. It is not a reason to skip the day.` },
      { p: `I do the deciding the night before, so the session is only execution.` },
      { p: `And I grade the week on those three process questions before anyone looks at the profit and loss.` },
      { h: `Common mistakes` },
      { p: `[b]Waiting for the perfect market, the perfect base and the perfect entry price[/b] while the leaders run without you — and the month ends flat with a notebook full of near-misses.` },
      { p: `[b]Treating a green week as proof of good process and a red one as proof of bad.[/b]` },
      { p: `[b]Only looking at your list of missed trades when it makes you feel better.[/b]` },
    ],
    take: `Done beats perfect. Take the trade the system flags, honour the stop, log it — and let the repetitions do what being clever cannot.`,
  },
];
...
const PLAYBOOK_CARD = [
  ...
  [`Managing the trade`, `At +3R–5R (or day 3–5 as the fallback): sell 30–50%, move the stop to break-even · when it stretches far above the 50-day, sell into the move — never short it · trail the rest on one moving average, daily closes only, forgiving quick dips · add only at a new pause, with the stop moving up.`],
  [`No-buy — any yes kills it`, `Market check red? · Group not leading, or only bouncing off its lows? · Stretched? · Loose base? · No volume on the break? · Earnings within 5 days? · Open risk already at the cap? · Trading a feeling?`],
  ...
  const card = `<div class="page pbpiece pcRule" id="card" style="max-width:1060px">
    <div class="pbgrid">
      ...
      <div style="min-width:0">
        ...
        <div class="pcD pbsub">Twenty-one pieces, compressed to what you check before the click.</div>
        <div class="pcRule pbhair"></div>
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>21 Building Blocks to Profitability</title>
    ...
    <style>
      *{box-sizing:border-box;margin:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      ...
      /* NEVER nowrap here. A key longer than the 230px track (e.g. "Revenue growth, quarterly YoY
         > 25%", uppercased + letter-spaced) overflows its grid cell and prints ON TOP of the value
         column. That shipped once — Valen caught it 2026-08-02. Keys wrap; they do not overhang. */
      .pbkvk{font-size:0.7rem;letter-spacing:0.08em;text-transform:uppercase;padding-top:3px;line-height:1.55;overflow-wrap:anywhere;min-width:0}
      .pbkvv{font-size:0.95rem;line-height:1.7;min-width:0;overflow-wrap:break-word}
      ...
      /* Inline [a=…] link inside body copy. break-word (not break-all) so a long saved-screen URL
         wraps at a sane point instead of overhanging the reading column. */
      .pblink{color:#c9982a;text-decoration:none;border-bottom:1px solid rgba(201,152,42,0.4);padding-bottom:1px;overflow-wrap:break-word}
      body.light .pblink{color:#8a6a1c;border-bottom-color:rgba(138,106,28,0.4)}
    ...
    <div class="zoomov" id="zoomov"><img id="zoomim" alt=""/></div>
    <script>
    // Click-to-zoom on screen; hidden in print so the saved PDF is untouched.
    document.addEventListener('click',(e)=>{const ov=document.getElementById('zoomov');
      if(e.target.closest('#zoomov')){ov.style.display='none';return;}
...
export default function ModelBookPage({ C, font, session, isAdmin, guideEnter, guideLeave, gactive, journaledTrades }) {
  ...
  // #playbook deep link (Valen 2026-08-02) — the Skool onboarding link. Opening a blob tab needs a
  // user gesture in most browsers, so we do NOT auto-open: we show a one-click card at the top of
  // the page. That also means the link degrades gracefully instead of being silently blocked.
  const [deepPlaybook, setDeepPlaybook] = useState(
    () => typeof window !== "undefined" && (window.location.hash || "").replace(/^#\/?/, "").toLowerCase() === "playbook");
  const consumePlaybookLink = () => {
    setDeepPlaybook(false);
    try { window.history.replaceState(null, "", "#modelbook"); } catch {}
    openPlaybookBook();
  };
  ...
  // Member self-serve dives (2026-08-03): member_dives rows — live ones (community shelf) + the
  ...
  // Registry dives take rows from the OFFICIAL account ONLY (2026-08-03): the community read
  // policy lets other members' live-dive rows into this fetch, and a member project named like a
  // registry project must never inject rows into the VIV shelf.
  const OFFICIAL_UID = "0e32b092-029a-436d-8cb5-67621e1467b0"; // vc-lv@live.com
  const diveList = DEEP_DIVES.map(d => ({ ...d, rows: d.static ? [] : rows.filter(r => isStudyRow(r) && studyProject(r) === d.project && r.created_by === OFFICIAL_UID) }))
  ...
  const myDive = memberDives.find((d) => d.created_by === uid) || null; // v1: one dive per member (first project wins the modal)
  const communityDives = memberDives.filter((d) => d.is_live && d.created_by !== "0e32b092-029a-436d-8cb5-67621e1467b0");
  const saveMemberDive = async (fields) => {
    const row = { ...fields, created_by: uid, updated_at: new Date().toISOString() };
    const q = myDive
      ? supabase.from("member_dives").update(row).eq("id", myDive.id)
      : supabase.from("member_dives").insert(row);
    const { error } = await q;
    if (error) { alert("Could not save the dive: " + error.message); return; }
    setDiveModal(false); loadMemberDives();
  };
  const deleteMemberDive = async () => {
    if (!myDive) return;
    await supabase.from("member_dives").delete().eq("id", myDive.id);
    setDiveModal(false); loadMemberDives();
  };
    ...
    <div style={{ fontFamily: font }}>
      {loading ? <div style={{ color: C.muted, fontSize: "0.8rem", padding: "30px 0" }}>Loading…</div>
        : <DiveIndex C={C} font={font} dives={diveList}
            ...
            community={memberDives.filter((d) => d.is_live || d.created_by === uid)} onOpenCommunity={openCommunityDive} uid={uid} />}
    </div>
  );
    ...
    <div style={{ fontFamily: font }}>
      ...
      {fScope === "dives" && !loading && !error && (
        <DiveIndex C={C} font={font} dives={diveList} onOpen={(d) => d.static === "playbook" ? openPlaybookBook() : openMyBookPdf(d.rows, { coverTitle: d.project })} isAdmin={isAdmin} onTogglePublish={toggleDivePublish}
          community={communityDives} onOpenCommunity={openCommunityDive} uid={uid} />
      )}
