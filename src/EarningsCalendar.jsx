...
const pctCol = (v) => (v == null || !isFinite(v) ? "var(--muted)" : v > 0 ? "var(--green)" : v < 0 ? "var(--red)" : "var(--muted)");
...
const STACKS = [
  { key: "bmo", label: "Before Open", icon: "☀", fg: "var(--blueFg)" },
  { key: "amc", label: "After Close", icon: "🌙", fg: "var(--purple)" },
  { key: "tbc", label: "Time TBC", icon: "—", fg: "var(--faint)" },
];
...
const TIMING = {
  bmo: { glyph: "☀", word: "Before open", fg: "var(--blueFg)", bg: "rgba(59,158,255,0.15)", bd: "rgba(59,158,255,0.4)", disc: "rgba(59,158,255,0.85)" },
  amc: { glyph: "🌙", word: "After close", fg: "var(--purple)", bg: "rgba(168,130,255,0.15)", bd: "rgba(168,130,255,0.4)", disc: "rgba(124,92,255,0.85)" },
  tbc: { glyph: "", word: "Time TBC", fg: "var(--faint)", bg: "var(--w06)", bd: "var(--w10)", disc: "var(--w22)" },
};
...
function TimingChip({ time }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "2px 9px", borderRadius: 999, background: m.bg, border: `1px solid ${m.bd}`, color: m.fg, fontSize: "0.6875rem", fontWeight: 600, letterSpacing: "0.05em", whiteSpace: "nowrap", lineHeight: 1.5 }}>
      {m.glyph && <span style={{ fontSize: "0.875rem", lineHeight: 1 }}>{m.glyph}</span>}
...
const howLines = [
  ["On your radar", "the leaders you already track, laid out day by day, with the day's biggest other reporters filling in quietly — a fast scan for who reports soon."],
...
// leaders as starred chips, fillers as quiet neutral chips, and past-day rows in their reported state.
...
// starred; the day's biggest other reporters fill in as quiet neutral chips (never a star).
//
...
function UpcomingList({ daysMap, today, held, C, days: maxDays = 4, perDay = 5 }) {
  ...
  if (!cols.length) return <div style={{ padding: "12px 4px", fontSize: "0.6875rem", color: C.muted }}>Nothing reports in the days ahead.</div>;
      ...
      {cols.map(({ d, rows, more }) => (
        <div key={d} style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 5, paddingBottom: 5, marginBottom: 6, borderBottom: `1px solid ${C.border}` }}>
            <span style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: d === today ? C.white : C.muted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {d === today ? "Today" : dl(d)}
            </span>
            {more > 0 && <span style={{ marginLeft: "auto", fontSize: "0.6875rem", fontWeight: 700, color: C.muted, opacity: 0.65 }}>+{more}</span>}
          </div>
            ...
            {rows.map((r) => {
              ...
              return (
                <div key={r.t} style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                  <span title={timeWord(r.time)} style={{ flex: "none", width: 11, textAlign: "center", fontSize: "0.6875rem", color: m.fg }}>{m.glyph || "·"}</span>
                  <span style={{ fontSize: "0.75rem", fontWeight: 600, color: r.held ? "var(--blueFg)" : "var(--text)", overflow: "hidden", textOverflow: "ellipsis" }}>{r.t}{r.ldr && <span style={{ color: C.white, fontSize: "0.6875rem" }}> ★</span>}</span>
                  {r.held && (
                    <span title="You hold this — a report can gap it overnight"
                      style={{ flex: "none", fontSize: "0.6875rem", fontWeight: 600, letterSpacing: "0.06em", padding: "1px 4px", borderRadius: 4, background: "rgba(96,165,250,0.18)", border: "1px solid rgba(96,165,250,0.45)", color: "var(--blueFg)" }}>HOLD</span>
                  )}
                </div>
              );
            })}
  ...
  const chip = (r, d, showQ, past) => {
    ...
    return (
      <div key={r.t} className={"radar-chip" + (leader ? "" : " nlr") + (interactive ? "" : " ni")}
        ...
        {past && valTxt && <span className="ra" style={{ color: reported ? beatCol(r) : "var(--faint)" }}>{valTxt}</span>}
  ...
  return (
    <div className="radarstrip">
      <style>{`
        ...
        .radarstrip .hscroll::-webkit-scrollbar-thumb{background:var(--w10);border-radius:3px}
        .radarstrip .radar{display:flex;gap:8px;width:100%;padding-bottom:4px;align-items:stretch}
        .radarstrip .radar-day{flex:1 1 0;min-width:150px;display:flex;flex-direction:column;padding-right:8px;border-right:1px dotted var(--w10)}
        .radarstrip .radar-day.last{border-right:none;padding-right:0}
        .radarstrip .radar-day.past{opacity:0.5}
        .radarstrip .radar-hd{font-size:0.75rem;font-weight:600;letter-spacing:0.005em;color:${C.white};text-align:center;padding:0 2px 7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border-bottom:1px solid var(--w08);margin-bottom:8px}
        .radarstrip .radar-hd.today{color:${C.white}}
        .radarstrip .radar-macro{text-align:center;font-size:0.75rem;color:var(--blueFg);margin:-4px 0 6px;cursor:help;line-height:1}
        ...
        .radarstrip .sub-h{display:flex;align-items:center;gap:4px;font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;padding:2px 6px;border-radius:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .radarstrip .radar-chip{display:flex;align-items:center;gap:3px;background:var(--w08);border:1px solid var(--w22);border-radius:7px;padding:4px 7px;cursor:pointer;overflow:hidden}
        .radarstrip .radar-chip .tk{font-size:0.875rem;font-weight:600;color:${C.white};white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .radarstrip .radar-chip .q{flex:none;font-size:0.6875rem;font-weight:600;color:${C.muted}}
        .radarstrip .radar-chip .ra{flex:none;font-size:0.6875rem;font-weight:500;letter-spacing:-0.025em;font-variant-numeric:tabular-nums;white-space:nowrap}
        .radarstrip .radar-chip.nlr{background:var(--w06);border-color:${C.border}}
        ...
        .radarstrip .radar-reported .rep-h{font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;color:${C.muted};text-align:center;margin-bottom:2px}
        .radarstrip .radar-dim{color:var(--faint);font-size:0.875rem;text-align:center;padding:3px 0}
        .radarstrip .radar-tbc{margin-top:8px;padding-top:7px;border-top:1px solid var(--w06)}
        .radarstrip .radar-tbc .tbc-h{font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;color:${C.muted};margin-bottom:5px;text-align:center}
...
function SurpriseChip({ r, big }) {
  ...
  if (r.epsActual == null || r.surprisePct == null) {
    return <span style={{ fontSize: fs, fontWeight: 600, color: "var(--faint)" }}>—</span>;
  }
  const p = r.surprisePct, beat = p >= 0;
  const fg = p > 0 ? "var(--green)" : p < 0 ? "var(--red)" : "var(--muted)";
  const bg = p > 0 ? "rgba(0,200,5,0.12)" : p < 0 ? "rgba(255,80,0,0.10)" : "var(--w06)";
  const bd = p > 0 ? "rgba(0,200,5,0.35)" : p < 0 ? "rgba(255,80,0,0.3)" : "var(--w10)";
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: fs, fontWeight: 600, padding: "1px 7px", borderRadius: 999, background: bg, border: `1px solid ${bd}`, color: fg, whiteSpace: "nowrap" }}>
      {p === 0 ? "0%" : `${beat ? "✓" : "✗"} ${sgnPct(p, 1)}`}
    </span>
  );
}

// one ticker chip. `large` (top-3 by cap) → bigger; `leader` → ★. `reported` → beat/miss + reaction.
function TickerChip({ r, large, leader, reported, C, onClick }) {
  ...
  return (
    <div className={"ec-chip" + (large ? " lg" : "") + (leader ? " ldr" : "")} onClick={() => onClick && onClick(r)} title="View details">
      ...
      {reported && (r.rx || actA) && (
        <div className="ec-rx">
          {actA && <span><span style={{ color: "var(--faint)" }}>A </span><span style={{ color: beatCol(r), fontWeight: 500, letterSpacing: "-0.025em" }}>{actA}</span></span>}
...
// uppercase lev-ETF chips (neutral, mirrors the rotation tab's levCell).
function LevChips({ arr, C }) {
  if (!arr || !arr.length) return <span style={{ color: C.muted, fontSize: "0.75rem" }}>—</span>;
  return (
    <span style={{ display: "inline-flex", gap: 5, flexWrap: "wrap" }}>
      {arr.map((e) => (
        <span key={e} style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: "0.6875rem", fontWeight: 600, color: C.white, letterSpacing: "0.03em", background: "var(--w06)", border: "1px solid var(--w22)", borderRadius: 6, padding: "2px 7px" }}>{e.toUpperCase()}</span>
      ))}
    </span>
  );
}
...
function EarningsDetailPopup({ target, C, font, onClose }) {
  ...
  const label = { fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--faint)" };
  const statBox = { padding: "9px 11px", borderRadius: 10, background: "var(--w03)", border: `1px solid ${C.border}` };
  const statVal = { fontSize: "0.875rem", fontWeight: 500, letterSpacing: "-0.025em", color: C.white, fontVariantNumeric: "tabular-nums", marginTop: 3 };
  const rxBox = (lbl, v) => (
    <div style={{ flex: 1, textAlign: "center", padding: "7px 6px", borderRadius: 9, background: "var(--w03)", border: `1px solid ${C.border}` }}>
      <div style={{ fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>{lbl}</div>
      <div style={{ fontSize: "0.875rem", fontWeight: 500, letterSpacing: "-0.025em", marginTop: 2, color: pctCol(v), fontVariantNumeric: "tabular-nums" }}>{sgnPct(v)}</div>
    </div>
  );
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1320, background: "var(--scrim)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", overflowY: "auto", padding: "40px 16px", fontFamily: font }}>
      <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 440, margin: "0 auto", background: "var(--w04)", border: "1px solid var(--w10)", borderRadius: 16, backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", boxShadow: "var(--shadowLg)", overflow: "hidden" }}>
        {/* header */}
        <div style={{ padding: "16px 18px 14px", borderBottom: `1px solid ${C.border}`, background: "linear-gradient(135deg,var(--w06),transparent 60%)" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 9, minWidth: 0 }}>
              <span style={{ fontSize: "1.125rem", fontWeight: 600, color: C.white, letterSpacing: "-0.01em" }}>{r.t}</span>
              {leader && <span title="One of the liquid leaders" style={{ fontSize: "0.6875rem", fontWeight: 600, color: C.white, background: "var(--w10)", border: "1px solid var(--w22)", borderRadius: 999, padding: "2px 8px" }}>★ Liquid leader</span>}
            </div>
            <button onClick={onClose} title="Close" style={{ flex: "none", width: 38, height: 38, display: "grid", placeItems: "center", padding: 0, borderRadius: 8, border: `1px solid ${C.border}`, background: "var(--w03)", color: C.muted, fontSize: "0.875rem", cursor: "pointer", lineHeight: 1, fontFamily: font }}>×</button>
          </div>
          {r.name && <div style={{ marginTop: 4, fontSize: "0.75rem", fontWeight: 600, color: C.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.name}</div>}
        </div>
        {/* body */}
        <div style={{ padding: "16px 18px 18px", display: "grid", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <span style={{ fontSize: "0.875rem", fontWeight: 700, color: C.white }}>{longDay(day)}</span>
              <TimingChip time={r.time} />
            </div>
            <div style={{ marginTop: 5, fontSize: "0.6875rem", lineHeight: 1.5, color: "var(--faint)" }}>Scheduled — dates can shift until the company confirms.</div>
          </div>
          ...
          {!reported && (
            <>
              ...
              {r.noEsts != null && <div style={{ fontSize: "0.6875rem", color: "var(--faint)" }}>Consensus drawn from {r.noEsts} analyst estimate{r.noEsts === 1 ? "" : "s"}.</div>}
            </>
          )}
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                ...
                {(() => { const bc = beatCol(r); const beat = bc === GREEN, miss = bc === RED; return (
                  <div style={{ ...statBox, border: `1px solid ${beat ? "rgba(0,200,5,0.35)" : miss ? "rgba(255,80,0,0.3)" : C.border}`, background: beat ? "rgba(0,200,5,0.07)" : miss ? "rgba(255,80,0,0.06)" : statBox.background }}>
              ...
              <div style={{ padding: "10px 13px", borderRadius: 10, background: "var(--w03)", border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: "0.6875rem", lineHeight: 1.55, color: C.muted, fontWeight: 500 }}>After a report, moves tend to resolve one of three ways: an immediate breakout, a quiet range that breaks later, or a reversal when the surprise was already priced in.</div>
              </div>
            </>
          )}
          ...
          {!reported && (
            <div style={{ padding: "10px 13px", borderRadius: 10, background: diff >= 0 && diff <= 5 ? "rgba(255,170,5,0.08)" : "var(--w03)", border: `1px solid ${diff >= 0 && diff <= 5 ? "rgba(255,170,5,0.3)" : C.border}` }}>
              <div style={{ fontSize: "0.75rem", lineHeight: 1.55, color: diff >= 0 && diff <= 5 ? "var(--orange)" : C.muted, fontWeight: 500 }}>{windowLine}</div>
            </div>
          )}
          ...
          {leader && (
            <div style={{ display: "grid", gap: 9 }}>
              ...
              <div style={{ fontSize: "0.6875rem", lineHeight: 1.5, color: "var(--faint)" }}>Leveraged/inverse funds — liquidity varies, always check the fund before using it.</div>
            </div>
          )}
        </div>
      </div>
      ...
      {weeks.map((w) => (
        <section key={w.ws} className="earn-card">
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: C.muted, fontVariantNumeric: "tabular-nums" }}>{w.weekRowCount} {w.weekRowCount === 1 ? "report" : "reports"}</span>
          </div>
          {w.weekRowCount === 0 ? (
            <div className="ec-empty" style={{ fontSize: "0.75rem", padding: "18px 10px" }}>No scheduled reports this week in the current data.</div>
          ) : (
                    ...
                    {col.shown === 0 ? <div className="ec-empty">—</div> : STACKS.map((s) => {
                      ...
                      return (
                        <div key={s.key} className="ec-stack">
                          <div className="ec-stack-label"><TimingChip time={s.key} /><span style={{ color: "var(--faint)", fontWeight: 700 }}>· {list.length}</span></div>
...
export default function EarningsCalendar({ C, font, session }) {
  ...
  // ── style primitives ──
  const cardLabel = { fontSize: "0.75rem", fontWeight: 500, letterSpacing: 0, color: "var(--muted)" };
  const asofStyle = { fontSize: "0.6875rem", fontWeight: 700, color: C.muted, fontVariantNumeric: "tabular-nums", textAlign: "right" };
  const chipStyle = (active) => ({
    ...
    border: `1px solid ${active ? "var(--w14)" : C.border}`, color: active ? C.white : C.muted,
  ...
  return (
    <div ref={rootRef} className="earn" style={{ fontFamily: font, maxWidth: 1180, margin: "0 auto", color: C.text }}>
      <style>{`
        .earn .earn-card{position:relative;background:var(--w04);border:1px solid var(--w08);border-radius:16px;backdrop-filter:blur(24px) saturate(150%);-webkit-backdrop-filter:blur(24px) saturate(150%);padding:20px 22px;margin-bottom:12px}
        .earn .earn-card::before{content:'';position:absolute;inset:0;pointer-events:none;border-radius:inherit;background:linear-gradient(135deg,var(--w06),transparent 55%)}
        .earn .hscroll{overflow-x:auto;-webkit-overflow-scrolling:touch}
        .earn .hscroll::-webkit-scrollbar{height:6px}
        .earn .hscroll::-webkit-scrollbar-thumb{background:var(--w10);border-radius:3px}
        ...
        .earn .sr-row{display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:9px;background:var(--w02);border:1px solid var(--w06);cursor:pointer}
        .earn .sr-row:hover{background:var(--w06)}
        .earn .sr-day{flex:0 0 52px;font-size:0.6875rem;font-weight:600;color:${C.muted};white-space:nowrap}
        .earn .sr-tk{flex:0 0 64px;font-size:0.75rem;font-weight:600;color:${C.white};white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .earn .sr-ea{flex:0 0 108px;font-size:0.6875rem;font-weight:700;color:${C.muted};white-space:nowrap;font-variant-numeric:tabular-nums}
        .earn .sr-sp{flex:0 0 74px}
        .earn .sr-pct{flex:0 0 56px;text-align:right;font-size:0.6875rem;font-weight:500;letter-spacing:-0.025em;font-variant-numeric:tabular-nums}
        .earn .sr-h{font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;color:var(--faint)}
        ...
        .earn .sr-card .sr-tk{flex-basis:52px;font-size:0.6875rem}
        .earn .sr-card .sr-ea{flex-basis:86px;font-size:0.6875rem}
        .earn .sr-card .sr-sp{flex-basis:60px}
        .earn .sr-card .sr-pct{flex-basis:46px;font-size:0.6875rem}
        /* day pills */
        .earn .pills{display:flex;gap:7px;width:max-content;padding-bottom:4px;align-items:stretch}
        .earn .pill{flex:0 0 auto;display:flex;flex-direction:column;align-items:center;gap:1px;padding:6px 12px;border-radius:999px;cursor:pointer;font-family:inherit;border:1px solid ${C.border};background:var(--w03);transition:all .14s}
        .earn .pill.past{opacity:0.6}
        .earn .pill .p-wd{font-size:0.75rem;font-weight:600;color:${C.white};white-space:nowrap}
        .earn .pill .p-meta{font-size:0.6875rem;font-weight:500;color:${C.muted};white-space:nowrap}
        .earn .pill.on{border-color:var(--w14);background:var(--w10);opacity:1}
        .earn .pill.on .p-wd,.earn .pill.on .p-meta{color:${C.white}}
        .earn .pill-div{flex:0 0 auto;width:1px;align-self:stretch;background:var(--w14);margin:2px 3px}
        /* macro (FOMC/CPI) banner — slim blue-bordered row at the top of a day carrying a
           high-impact macro event; folded into the existing day panel / week-grid columns
           (no new dashboard card). */
        .earn .macro-banner{padding:8px 12px;border-radius:10px;border:1px solid rgba(147,197,253,0.35);background:rgba(59,158,255,0.10);color:var(--blueFg);font-size:0.75rem;font-weight:700;line-height:1.5}
        /* day panel */
        .earn .paneln{display:grid;grid-template-columns:1fr 1fr;gap:12px}
        .earn .pcol{border-radius:12px;background:var(--w02);border:1px solid var(--w06);overflow:hidden}
        .earn .pcolhd{padding:9px 12px;border-bottom:1px solid var(--w06);font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;display:flex;align-items:center;gap:6px}
        .earn .pcolbody{padding:9px;display:flex;flex-direction:column;gap:5px}
        .earn .pcolfoot{padding:6px 12px;border-top:1px solid var(--w06);font-size:0.6875rem;font-weight:700;color:var(--faint);font-variant-numeric:tabular-nums}
        .earn .tbc-block{margin-top:10px}
        /* chip */
        .earn .ec-chip{padding:5px 9px;border-radius:9px;background:var(--w04);border:1px solid var(--w08);cursor:pointer;transition:background .12s,border-color .12s}
        .earn .ec-chip:hover{background:var(--w08)}
        .earn .ec-chip.lg{padding:7px 10px;background:var(--w06);border-color:var(--w14)}
        .earn .ec-chip.ldr{border-color:var(--w22);background:var(--w08)}
        .earn .ec-chip.lg.ldr{border-color:var(--w35)}
        .earn .ec-chip-top{display:flex;align-items:baseline;justify-content:space-between;gap:6px}
        .earn .ec-tk{font-size:0.75rem;font-weight:600;color:${C.white};white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .earn .ec-chip.lg .ec-tk{font-size:0.875rem}
        .earn .ec-star{color:${C.white};font-size:0.6875rem}
        .earn .ec-val{flex:none;font-size:0.6875rem;font-weight:700;color:${C.muted};font-variant-numeric:tabular-nums}
        .earn .ec-name{margin-top:1px;font-size:0.6875rem;font-weight:500;color:var(--faint);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .earn .ec-chip.lg .ec-name{font-size:0.6875rem}
        .earn .ec-rx{margin-top:3px;display:flex;gap:7px;font-size:0.6875rem;font-weight:600;font-variant-numeric:tabular-nums}
        /* week grid */
        .earn .ec-grid{position:relative;display:flex;gap:10px}
        .earn .ec-col{flex:1 1 0;min-width:0;display:flex;flex-direction:column;border-radius:12px;background:var(--w02);border:1px solid var(--w06);overflow:hidden}
        .earn .ec-col.today{border-left:3px solid var(--w55);background:var(--w03)}
        .earn .ec-colhead{padding:9px 10px 8px;border-bottom:1px solid var(--w06);display:flex;align-items:baseline;gap:6px}
        .earn .ec-wd{font-size:0.75rem;font-weight:600;color:${C.white};letter-spacing:-0.01em}
        .earn .ec-dt{font-size:0.6875rem;font-weight:700;color:${C.muted};font-variant-numeric:tabular-nums}
        .earn .ec-todaytag{margin-left:auto;font-size:0.6875rem;font-weight:600;letter-spacing:0.06em;padding:2px 6px;border-radius:999px;background:var(--w14);color:${C.white}}
        .earn .ec-colbody{padding:8px;display:flex;flex-direction:column;gap:9px;flex:1}
        .earn .ec-stack-label{display:flex;align-items:center;gap:5px;font-size:0.6875rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:6px}
        .earn .ec-stack+.ec-stack{margin-top:2px;padding-top:8px;border-top:1px solid var(--w06)}
        .earn .ec-chips{display:flex;flex-direction:column;gap:5px}
        .earn .ec-colfoot{padding:6px 10px;border-top:1px solid var(--w06);font-size:0.6875rem;font-weight:700;color:var(--faint);font-variant-numeric:tabular-nums}
        .earn .ec-empty{padding:14px 10px;font-size:0.6875rem;color:var(--faint);text-align:center}
        @media (max-width:760px){
        ...
        @media (max-width:560px){.earn .earn-card{padding:16px 14px}}
      `}</style>
      ...
      <section className="earn-card" style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "flex-start", justifyContent: "space-between" }}>
        <div style={{ minWidth: 0 }}>
          ...
          <h1 style={{ margin: "0 0 5px", fontSize: "1.25rem", fontWeight: 600, letterSpacing: "-0.02em", color: C.white }}>Earnings calendar</h1>
          <p style={{ margin: 0, fontSize: "0.75rem", lineHeight: 1.55, color: C.muted, maxWidth: "70ch" }}>
      ...
      {note && (
        <section className="earn-card" style={{ padding: "11px 15px", fontSize: "0.75rem", lineHeight: 1.55, color: C.muted }}>
      ...
      <section className="earn-card">
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 11 }}>
          ...
          <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: C.muted }}>leaders starred · the day's biggest others fill in</span>
          <span style={{ marginLeft: "auto", fontSize: "0.6875rem", fontWeight: 700, color: C.muted, fontVariantNumeric: "tabular-nums" }}>{radarLeaderTotal} leaders in range</span>
          <SectionCamera sel=".earn-card" name="earnings-radar-full" C={C} style={{ position: "static", top: "auto", right: "auto" }} />
        </div>
        {!radarHasRows ? (
          <div className="ec-empty" style={{ fontSize: "0.75rem", padding: "14px 10px" }}>No reporters in this window.</div>
        ) : (
      ...
      {surpriseRows.length > 0 && (
        <section className="earn-card sr-card">
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 11, flexWrap: "wrap" }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: "0.6875rem", fontWeight: 700, color: C.muted }}>ranked by size of surprise</span>
            ...
            <div className="sr-rows">
              ...
              {surpriseRows.map((r) => (
                <div key={r.day + r.t} className="sr-row" onClick={() => openChip(r, r.day)}>
                  ...
                  <span className="sr-tk">{r.t}{isLeader(r.t) && <span style={{ color: C.white, fontSize: "0.6875rem" }}> ★</span>}</span>
                  <span className="sr-ea">{eps(r.epsEst) || "—"} → <span style={{ color: beatCol(r), fontWeight: 500, letterSpacing: "-0.025em" }}>{eps(r.epsActual) || "—"}</span></span>
          ...
          <div style={{ marginTop: 9, fontSize: "0.6875rem", lineHeight: 1.5, color: "var(--faint)" }}>
      ...
      <section className="earn-card">
        ...
        {view === "compact" ? (
          tradingDays.length === 0 ? (
            <div className="ec-empty" style={{ fontSize: "0.75rem", padding: "16px 10px" }}>No scheduled reports in the current data window.</div>
          ) : (
            <>
                ...
                <>
                    ...
                    {[STACKS[0], STACKS[1]].map((s) => {
                      ...
                      return (
                        <div key={s.key} className="pcol">
                          <div className="pcolhd"><TimingChip time={s.key} /><span style={{ color: "var(--faint)", fontWeight: 700 }}>· {list.length}</span></div>
                  ...
                  {(panel.stacks.tbc || []).length > 0 && (
                    <div className="pcol tbc-block">
                      <div className="pcolhd"><TimingChip time="tbc" /><span style={{ color: "var(--faint)", fontWeight: 700 }}>· {panel.stacks.tbc.length}</span></div>
                  ...
                  {panel.shown === 0 && <div className="ec-empty" style={{ fontSize: "0.75rem", padding: "14px 10px" }}>No reports match this filter on {longDay(activeDay)}.</div>}
                </>
              )}
            </>
          )
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 7 }}>
            {howLines.map(([k, v]) => (
              <div key={k} style={{ display: "flex", gap: 10, alignItems: "baseline", fontSize: "0.75rem", lineHeight: 1.5 }}>
                <span style={{ color: C.white, fontWeight: 600, flex: "none", minWidth: 156 }}>{k}</span>
        ...
        <section className="earn-card">
          ...
          {methodOpen && (
            <div style={{ marginTop: 12, display: "grid", gap: 10, maxWidth: "92ch" }}>
              ...
              <p style={{ margin: 0, fontSize: "0.75rem", lineHeight: 1.7, color: C.muted }}>
                <b style={{ color: C.text }}>Reactions</b> are computed from daily candles for past-day reporters only, bounded to each day's top 25 by market cap ∪ all liquid leaders. Reaction day = the report day for a before-open report, else the next trading day. Gap = open vs prior close · Session = close vs open · Total = close vs prior close. Below cap / bars missing → shown as "—". Chip prominence scales by market cap; leaders pin first. Refresh: <code style={{ color: C.text }}>node --env-file=.env.local scripts/earnings-fetch.mjs</code>.
              </p>
            </div>
          )}
        </section>
      ...
      <div ref={cardRef} className="card lensmini" onClick={() => setOpen(true)} style={{ fontFamily: font, cursor: "pointer" }}>
        <div className="cardhead">
          ...
          <InfoDot tip={{ lead: "Who reports, and when.", points: [
            "Liquid leaders (★) come first each day",
          ...
          {!noStamp && <span style={{ marginLeft: "auto", fontSize: "0.6875rem", fontWeight: 700, color: C.muted, fontVariantNumeric: "tabular-nums" }}>{stamp}</span>}
        </div>
        {compact ? (
          ...
          <div style={{ padding: "12px 4px", fontSize: "0.6875rem", color: C.muted }}>No reporters in the three-week window.</div>
        )}
      </div>
      {open && createPortal(
        <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 1250, background: "var(--scrim)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", overflowY: "auto", padding: "32px 16px" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 1220, margin: "0 auto", position: "relative" }}>
