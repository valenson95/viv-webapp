...
function TradeJournalPage({ setPage, onLogout, journaledTrades, setJournaledTrades, setupTypes, tags: allTags, exitReasons, session, onManualSave, saveStatus, positions, setPositions, positionsRef, portfolioSize, displayName }) {
  ...
  const eqSvg = useMemo(() => {
    const sorted = dateFiltered.slice().sort((a, b) => Date.parse(tradeDateISO(a.exit) || 0) - Date.parse(tradeDateISO(b.exit) || 0));
    // Cumulative equity series. By Date = one node per trade; By Month = one node per month-end
    // (trades aggregated into their exit month) so the curve actually reshapes when you toggle.
    const monthKeys = [];
    const eq = [startCap];
    if (eqXAxis === "months") {
      const monthSums = {};
      sorted.forEach(t => {
        const m = tradeDateISO(t.exit || t.entry).slice(0, 7) || "Unknown";
        if (!(m in monthSums)) { monthSums[m] = 0; monthKeys.push(m); }
        monthSums[m] += (Number(t.plDollar) || 0);
      });
      monthKeys.sort();
      monthKeys.forEach(m => eq.push(eq[eq.length - 1] + monthSums[m]));
    } else {
      sorted.forEach(t => eq.push(eq[eq.length - 1] + (Number(t.plDollar) || 0)));
    }
    ...
    let xs;
    if (eqXAxis === "months") { xs = monthKeys.map(m => { const d = new Date(m + "-15"); return isNaN(d) ? m : d.toLocaleString("default", { month: "short", year: "2-digit" }); }); }
    else {
