...
function PlaybookTracker({ trades, uid, setPage }) {
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem("viv-goto-playbook")) {
        sessionStorage.removeItem("viv-goto-playbook");
        setMore(true);
        setTimeout(() => document.getElementById("playbook-tracker")?.scrollIntoView({ behavior: "smooth", block: "start" }), 350);
      }
    } catch { /* private mode */ }
  }, []);
...
const WHATS_NEW = [
  ...
  },
  {
    ...
    items: [
      ...
      "Playbook tracker → More details: your playbook as designed vs your actuals projected over the next 100 trades — a live feedback loop between the system you planned and the one you're actually trading, now with a projected equity chart of both curves.",
      "Return Simulator: auto-fill buttons for starting/current capital, plus 🎯 Sync to Playbook — your simulated design becomes the Playbook tracker's benchmark — and Compare stats, which jumps you straight to the comparison.",
      "Trade details is now ONE clean TradingView-style review chart: your real ▲ entry / ▼ exit at the exact bars, stop line, timeframes, and drawing tools whose annotations auto-save per trade (never lost). Notes sit right below the chart. Replay was retired.",
      "Equity curve: all three SMAs are dotted and each toggles on/off from the legend; a fourth tier joined the ladder — equity below the 20-SMA = SYSTEM OFFLINE (smallest or sim-only size + full audit).",
  ...
  },
  {
    ...
    items: [
      "The three stop fields now say exactly what they do: Original stop (locked — drives your R) · 2nd stop = a DUAL stop that keeps half your shares on the original level · Trailing stop / BE = moves the WHOLE position's stop. Thanks Vincentius for the report that exposed the confusion.",
      "New '→ BE' button next to Trailing stop — one click sets it to your average cost. No more guessing which field means 'break-even'.",
      "If you set a 2nd stop at or above your cost, the app now warns you that half your shares are still on the original stop — with the exact share split shown under Risk to stop, so the number always explains itself.",
      "IBKR sync now understands currencies: non-USD trades convert to USD automatically using IBKR's own per-trade FX rate (US-only accounts are completely unaffected). Non-stock rows (forex/CFDs) no longer enter your journal. Thanks J.H. for the report.",
      "If your Flex template is missing the fxRateToBase field, foreign trades are safely skipped and the sync tells you how to add it — they'll never distort your stats again.",
      "As always: nothing in this update changes any of your saved trades, stops, or stats.",
    ],
  },
  {
    tag: "New",
...
function ReturnSimulatorTab({ guideEnter, guideLeave, gactive, expert, portfolioSize, currentCapital, session, setPage }) {
    ...
    <div className="card">
      <div className="io">
        <div>
          ...
          {(() => {
            const syncToPlaybook = (goCompare) => {
              if (!sim) return;
              const uid = session?.user?.id || "anon";
  ...
  return (
    <div className="reveal" style={{ marginTop: 18 }}>
      ...
      {false /* Goldilocks zone hidden per Valen (2026-07-04) */ && data.goldilocks && (() => { const g = data.goldilocks;
        ...
        const N = 10;
        const WRS = [30, 40, 50, 60, 70];                          // columns = win rate
        ...
        <div style={{ background: "rgba(201,152,42,0.05)", border: `1px solid ${C.borderGold}`, borderRadius: 12, padding: "16px 18px", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 14 }}>
            ...
            {tile("Your R:R", (g.gl ?? "—") + ":1", Number(g.gl) >= Number(g.breakeven_gl) ? C.green : C.red)}
        ...
        <div className="metrics">
          {(() => {
            ...
            const tiles = [
              ...
              { k: "pf", label: "Profit factor", val: m.n ? (isFinite(m.pf) ? m.pf.toFixed(2) : (m.wins ? "∞" : "—")) : "—", cls: "", sub: "gross win ÷ loss", tip: "TOTAL profit from all winners ÷ TOTAL loss from all losers — frequency baked in. Not the same as Win/Loss ratio (which compares only the AVERAGE win vs average loss) or the Playbook's payoff in R. Above 1.0 = the system makes money." },
...
function DashboardPage({ setPage, onLogout, onJournalTrade, setupTypes, tags: allTags, exitReasons, positions, setPositions, portfolioSize, setPortfolioSize, lastLoadedCountRef, lastSaveIdMapRef, session, targetRote, setTargetRote, journaledTrades, setJournaledTrades, onManualSave, saveStatus, positionsRef, saveErrorMsg, onIbkrSync, intradayColumnAvailable, intradayFeatureEnabled, onRunIntegrity, integrityReport, integrityRunning, displayName }) {
  ...
  const enriched = useMemo(() => positions.map(p => { try {
    ...
    const hasStop = parseFloat(p.stop_price) > 0; // a managed swing — excludes stopless core holdings (e.g. IBKR)
    const t3Due = tradingDaysHeld != null && tradingDaysHeld >= 3 && sharesN > 0 && !trimmedYet && hasStop;
