...
function TradeJournalPage({ setPage, onLogout, journaledTrades, setJournaledTrades, setupTypes, tags: allTags, exitReasons, session, onManualSave, saveStatus, positions, setPositions, positionsRef, portfolioSize, displayName }) {
  ...
  const eqSvg = useMemo(() => {
    ...
    if (eqXAxis === "months") { xs = []; sorted.forEach(t => { const m = tradeDateISO(t.exit || t.entry).slice(0, 7); if (m && xs.indexOf(m) < 0) xs.push(m); }); }
      ...
      const ds = sorted.map(t => { const p = (tradeDateISO(t.exit) || tradeDateISO(t.entry) || "").split("-"); return p.length === 3 ? (+p[1]) + "/" + (+p[2]) : ""; });
    ...
    if (sellAddJournal && soldShares > 0 && exitP > 0) {
      ...
      if (session) {
        ...
        const dbRow = {
          ...
          // position_id intentionally NOT written here: trades.position_id has a FK to positions(id),
          // and positions are re-created with new ids on every bulk Save, so pos.id is frequently a stale
          // id → the INSERT would fail the FK check (code 23503) and the whole trade would be lost.
          // The auto-link is preserved separately via saveTradeLinks()/trade_links and restored by applyTradeLinks().
          position_id: null,
        };
...
function AppInner() {
  ...
  const pageContent = (
    <>
      ...
      {page === "tools" && <PremiumToolsPage setPage={setPage} onLogout={handleLogout} session={session} demo={false} portfolioSize={portfolioSize} journaledTrades={journaledTrades} displayName={displayName} />}
