...
.vd .posrow td.mgcell{grid-column:1 / -1; order:99; flex-direction:row; gap:8px; padding:12px 0 2px; margin-top:6px; border-top:1px solid var(--border)}
          ...
          <table>
            <thead>
              <tr>
                ...
                <th className="pro-only"><span className="term tipright" data-tip="Profit already banked from partial sells of THIS position, with the share of the position you've sold. Scoped to this exact lot — old trades in the same ticker don't count.">Realized</span></th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              {enriched.filter(p => p.sym || p.id === manageId).map(p => {
                ...
                // Realized return % on the shares already sold out of this lot (banked gain ÷ cost basis of sold shares).
                const realizedPct = (p.realizedShares > 0 && p.epN > 0) ? (p.realizedPL / (p.realizedShares * p.epN)) * 100 : 0;
                return (
                  <React.Fragment key={p.id}>
                    <tr className={"posrow" + (isOpen ? " mg-open" : "")}>
                      ...
                      <td className="pro-only" data-l="Realized">{p.realizedShares > 0 ? (
                        <span className="sizebar" title={`${p.realizedShares} of ${p.origShares} shares sold (${p.trimPct.toFixed(0)}%)`}>
                          <span className="track"><span className="fill" style={{ width: Math.max(6, Math.min(100, p.trimPct)) + "%", background: p.realizedPL >= 0 ? "var(--green)" : "var(--red)" }}></span></span>
                          <small><span style={{ color: p.realizedPL >= 0 ? "var(--green)" : "var(--red)", fontWeight: 700 }}>{usdSigned(p.realizedPL)}</span> · {(realizedPct >= 0 ? "+" : "") + realizedPct.toFixed(2) + "%"}</small>
                        </span>
                      ) : <span style={{ color: "var(--faint)", fontSize: "0.7rem" }}>—</span>}</td>
