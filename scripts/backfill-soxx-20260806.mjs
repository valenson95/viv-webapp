// One-shot: backfill SOXX daily bars into scripts/.grouprs-cache.json so the new
// ["SOXX","Semiconductors"] UNIVERSE row computes (Hunt List coverage audit 2026-08-06).
...
const r = await fetch(`${PROXY}?symbol=SOXX&from=${FROM}&to=${today}&res=1day`);
const j = await r.json();
if (!j.ok || !j.candles?.length) throw new Error(`SOXX fetch failed: ${j.error || "no bars"}`);
...
console.log(`SOXX: ${bars.length} bars, ${bars[0].d} → ${bars[bars.length - 1].d}`);
...
const i = cache.raw.findIndex(x => x.t === "SOXX");
const entry = { t: "SOXX", name: "Semiconductors", bars };
