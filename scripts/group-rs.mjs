...
function computeRaw(b) {
  ...
  // Thrust stand-in, SAME machinery: weighted 1-week RS level (weights 5..1 on the last 5 daily
  // rel steps, /15), then PERCENTRANK of today's weighted-week value within its own trailing 25
  // values of that same measure (his stock sheet uses a 25-cell range). His published number adds
  // proprietary "weightage and bonus scoring" (prints −8…106) — that layer is UNPUBLISHED, so
  // ours stays bounded 0–100: read direction/divergence, not decimals.
  const W = [5, 4, 3, 2, 1];
  const wweek = (i) => { // weighted 5-session relative return ending at index i
    let s = 0;
    for (let k = 0; k < 5; k++) s += W[k] * (rel[i - k] / rel[i - k - 1] - 1);
    return s / 15;
  };
  const w5 = wweek(last);
  let thrustOwn = null;
  if (n >= 31) { // need 25 wweek values, each needing 6 bars of history
    const series = [];
    for (let i = last - 24; i <= last; i++) series.push(wweek(i));
    thrustOwn = prank(series, series[series.length - 1]) * 100;
  }
